...
| `key_grabbed` | `level, which` | Mario grabbed a Bowser key. `which` is `"bitdw"` (Bowser 1 arena, level 30) or `"bitfs"` (Bowser 2 arena, level 33). The same star-dance actions fire for keys and stars; the key detector claims both arenas and `star_grab.py` carries the inverse guard so keys are never journaled as misattributed `star_collected` events. B3's grand star (level 34) is a real star and stays with `star_grab`. |
...
| `target_set` | `kind, course_id, star_id, strat_tag?` **or** `kind, segment_id` | User explicitly set the practice target. `kind: "star"` carries `course_id`/`star_id`; `kind: "segment"` carries `segment_id`. Legacy payloads without `kind` = star. |
...
- Bowser-stage key grabs (Bowser 1/2 arenas) emit `key_grabbed`, not `star_collected`. B3's grand star (arena level 34) emits `star_collected` with attribution from `gLastCompletedCourseNum/StarNum` — VERIFY note in `addresses.py` covers what the game writes at that moment.
