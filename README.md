...
| `practice_reset` | `igt_frames_before, mario_acted, paused_frames_before, acted_tracking, action` | Usamune level reset — attempt anchor; payload carries the failed attempt's IGT and whether Mario entered any non-passive action since the last anchor (no-op resets where `mario_acted: false` are discarded); closures after ≥5 s of pause (paused_frames_before ≥ 150) are discarded as AFK; `action` = Mario's action at the detection tick — door/spawn context lets consumers classify load echoes (the segment engine ignores anchors where `action` is a door action: `0x1320/0x1321/0x1322`, as inputs are locked during door animations and Usamune resets IGT on every door crossing) |
| `state_loaded` | `igt_frames_restored, mario_acted, paused_frames_before, acted_tracking, action` | Savestate / Usamune section-state load — attempt anchor; same activity flag as practice_reset; same pause/activity discards as practice_reset; same `action` field as practice_reset for symmetry |
