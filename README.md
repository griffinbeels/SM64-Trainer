...
| `GET` | `/api/marelo` | `?scope=<id>` (optional) | `{scope_id, label, marelo, mastery, coverage, tier, division, next_division_at, division_progress, n, practiced, entities:[{key,label,score,tier,division,gain,excluded}], celebration, entity_celebrations:[{entity,label,from,to,tiers_gained,key}]}` for one scope (defaults to the active route, else Overall). `404` for an unknown scope. `entity_celebrations` evaluates the FULL rankable corpus (~102 entities), not just this scope's candidates — a star or segment outside the requested scope still reports here if it crossed a tier, because per-entity rank-ups are the frequent reward the feature exists to deliver (the aggregate `celebration` moves glacially by design). Same never-raise-on-GET contract as `celebration`: only `/api/marelo/ack` raises a watermark, so an un-acked rise keeps reporting on every fetch. |
