...
| `practice_reset` | `igt_frames_before, mario_acted, paused_frames_before, acted_tracking, action, prev_action` | Usamune level reset — attempt anchor; payload carries the failed attempt's IGT and whether Mario entered any non-passive action since the last anchor (no-op resets where `mario_acted: false` are discarded); closures after ≥5 s of pause (paused_frames_before ≥ 150) are discarded as AFK; `action` = Mario's action at the detection tick; `prev_action` = Mario's action on the PREVIOUS poll tick — the segment engine keys the door-echo clause on `prev_action`: a genuine door crossing has `prev_action` in `DOOR_ACTIONS` (inputs locked during the door animation on the prior tick); an L-reset respawning AT a door has a gameplay `prev_action` (e.g. freefall) and must be treated as a real reset. Fall back to `action` for events journaled before `prev_action` was added; missing both fields → conservative close. |
| `state_loaded` | `igt_frames_restored, mario_acted, paused_frames_before, acted_tracking, action, prev_action` | Savestate / Usamune section-state load — attempt anchor; same activity flag as practice_reset; same pause/activity discards as practice_reset; same `action` and `prev_action` fields as practice_reset for symmetry |
...
| `segment_disarmed` | `segment_id, name` | A segment's timer was stopped without recording an attempt (foreign level change, or silent disarm after a success). **Broadcast-only — never journaled.** |
...
| `target_changed` | `kind, course_id, star_id, strat_tag` **or** `kind, segment_id, segment_name` | Practice target moved (auto-follows last valid grab, or set by command). Same kind-aware shape as `target_set`. |
