...
// one level deeper, wrapped one more `.lib-cat` > `.lib-group` deep, and
// index.html's CSS keys off that nesting (not a --depth property — see its
// comment), so a third level needs no new class and no new rule.
...
export function GroupedList({ tree, open, toggle, renderRow, forceOpen, depth = 0 }) {
  return html`
    ...
    ${tree.children.map((node) => {
      const forced = forceOpen && forceOpen(node);
      const shut = !open.has(node.key) && !forced;
      // While forced open (a search hit), the group renders open regardless
      // of the stored set, so a click here does nothing VISIBLE — but
      // toggle() still ADDS the key to the persisted open-set, and the user
      // finds the group open after clearing the search even though nothing
      // looked toggled (review M5). Ignore the click while forced.
      return html`<div class="lib-cat" key=${node.key}>
        <button class=${`lib-cat-header ${shut ? "shut" : ""} ${depth > 0 ? "sub" : ""}`}
            aria-expanded=${!shut} onclick=${() => { if (!forced) toggle(node.key); }}>
