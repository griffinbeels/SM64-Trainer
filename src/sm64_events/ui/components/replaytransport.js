...
export function ReplayTransport({ playing, onStart, onStep, onToggle,
  ...
  const stepTitle = (direction) => canStep
    ? `Pause and move ${direction} one ${frameKind} frame`
  ...
  return html`<div class="replay-transport">
    ...
    <button onclick=${() => onStep(-1)} disabled=${!canStep} title=${stepTitle("back")}>
    ...
    <button onclick=${() => onStep(1)} disabled=${!canStep} title=${stepTitle("forward")}>
