...
- **Always-loaded project files** (AGENTS.md, architecture.md) — read/grepped
...
| A rule a machine can check (forbidden pattern, required pairing) — *especially one that has bitten more than once* | **Hook / lint / CI check** | A `.Codex/hooks/` script wired in `settings.json` — write it THIS pass, don't downgrade to prose (see below) |
...
| A workflow/process lesson (how Codex should work) | **By reliable channel: hook → always-loaded pointer → skill body** | Mechanical rule → a hook (fires every call). Otherwise the lesson's reliable surface is always-loaded: a lean AGENTS.md/AGENTS pointer, or a memory whose index hook stands alone. Edit a **skill body only if that skill reliably triggers at this decision point** — skill bodies load on a trigger that often doesn't fire, so they're weaker than the always-loaded index. Never a grab-bag "lessons" skill. |
...
(AGENTS.md), apply a lean budget — pointer lines, not paragraphs.
...
- Add a small script under `.Codex/hooks/` and wire it in `.Codex/settings.json`
...
- ✅ Nameable: "the test runner runs it"; "it's in AGENTS.md, always loaded";
...
<!-- Mirror: a copy of this skill is versioned in repos that adopt it
     (e.g. sm64_tracker/.Codex/skills/create-artifacts/). Keep in lockstep. -->
