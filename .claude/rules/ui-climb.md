...
paths:
  ...
  - "src/sm64_events/ui/routeswap.js"
...
**Every bar value inside a climb plan is a DRAWN WIDTH, not the server's
within-division `fill`.** `rankclimb.js` converts once, at the plan's boundary
(`targetBar`/`startBar` → `buildClimbPlan`'s `fromFill`/`toFill`), because a
rank bar is anchored at the track's midpoint for every rank but the ladder
floor (`caps.js::barFill`, `.claude/rules/ui-ranks.md`). That placement is
load-bearing and was got wrong first: `climbplan.js` builds the closing sweep
as `arrive: barFrom 0 → barTo`, and with raw fills going in that `0` means
"the bottom of this division", which the anchoring then paints HALF FULL —
"when we fill up the meter on the final beat of the animation, it STARTS AT
50% visually, which is wrong. It should START AT 0% visually… but ALWAYS END
PAST 50%" (user, 2026-07-29). With drawn widths going in, `0` means EMPTY and
the sweep is right by construction, the same way the mid-climb pin is
`barFrom === barTo === 1` rather than a case the walker remembers. Two knock-on
facts: a sweep's DURATION now scales with the distance it actually travels on
screen (`climbcurve.js::barSweepMs` reads `|barTo − barFrom|`), and the hook
hands out `climb.bar` — there is no `climb.fill` to pick by mistake.
`tests/test_ui_rank_bar.py::test_the_climb_anchors_the_bar_it_hands_out` fails
if either plan endpoint stops resolving through `barFill`.

**The "X.XXs to rank up" line may only CHANGE while it is INVISIBLE, and it
fades in EVERY case — including a climb that gains no rank at all** (user,
2026-08-01: "It should ALWAYS fade… even if we don't rank up"). `rankclimb.js`
publishes `climb.line` (`rest` / `leaving` / `climbing` / `arriving`) plus
`climb.atTarget`, and `ranks.js` resolves each to a sentence — so the rule is
answered in ONE place instead of by a condition per symptom, which is how it
regressed three times. Each was a single frame, invisible to every end-state
check: the second banner in a lane read 0 while WAITING and so cut 1→0→1 either
side of its wait; a no-rank-up plan is one bar step with no approach and no
arrival, so the line grew its suffix at full opacity (it exchanges inside that
one step now, via `climbcurve.js::exchangeFade`); and the render between a new
payload landing and the effect starting the climb still holds the OLD climb
state, which printed the destination for one frame at opacity 1 — `atTarget` is
resolved at RETURN time, against the props of the render asking, precisely
because inside the loop it would be a stale true. The fade rides the bar's own
progress — that tie is what stops the two drifting — but on its OWN clock, with
a floor (`revealMinMs`, a climbtuning row) the bar does not have: a sweep's
length scales with the distance it travels and legibility does not, so 0.10s
off a PB moved the bar ~2% of a division and faded in 100ms each way ("the
animation was still super snappy"). Above the floor it is byte-identical to the
bar's progress, so every climb big enough to see is untouched; the loop keeps
ticking while the line is still fading, or the resting state would hand it a
cut at the end of the very fade that exists to prevent one. Guarded by
`tests/test_ui_rank_line.py`, a frame trace over the inspector's own two
climbs, mutation-proved against all four. **The inspector can now play a
same-rank climb** (destination == start) and its fake payload's gap shrinks
with the bar — before that, the most common climb in the app was the one case
the demo could not show.

| To change... | Edit |
|---|---|
| Whose turn it is to celebrate (strategy → star → MARELO) | `ui/mareloturnstate.js` — the DECISION, import-free and node-tested (`tests/test_ui_marelo_turn.py`); `ui/mareloturn.js` is the hook around it, read by BOTH the header card (`header.js`) and the overlay (`marelocelebrate.js`) so they cannot disagree. Two things wait, not one: the overlay must not fly, and the header card must not quietly climb — it reads `t.marelo` directly. Both bugs here were single TRANSITIONS and both shipped past green source scans, which is why the logic left the hook: gating the overlay on "is anything climbing" LOOPED (the overlay renders a card that climbs, so mounting it made the gate true), and the held before-state was captured on the render the celebration ARRIVED — a payload that already carries the new rank, so the hold was a no-op and the card climbed anyway ("I THOUGHT i fixed a problem where the MARELO display… prematurely fires", 2026-08-01). `held` is now the last payload seen with NOTHING pending, `ready` is LATCHED per key, and the 450ms banners-appear grace is STATE rather than a ref — as a ref nothing could wake the effect that read it, so a celebration earned on a tab with no rank banner mounted froze the card on its before-state indefinitely. **A celebration that was already PENDING when a client arrived is acked without being shown** (`unprompted`) — the server holds a scope rank-up until acked, so one earned before the app closed greeted the next page load: "This should NEVER be triggered outside of updating a PB… it feels like a bug (because I didn't trigger it)" (2026-08-01). `held == null` says exactly that and nothing else, since `held` is written on every render with nothing pending. Same ack-without-showing path the celebrations-off preference uses, so the watermark still advances |
| Celebration overlays (the SCOPE rank) | `ui/components/marelocelebrate.js`'s `MareloCelebration` — ONE overlay for every overall rank-up since 2026-07-28, replacing celebrate.js's two hand-rolled scope treatments (`TierRankUp`'s fill→flip→hold, `DivisionRankUp`'s top banner), which this row had flagged as a KNOWN DEVIATION from 2026-07-26 until this landed. User, 2026-07-28: "it should maybe even animate from its current position to overlay very big in the center of the screen first, and THEN do the celebratory animation, and then animate back to its normal position." The thing that flies is the REAL `RouteRankCard` (`components/marelo.js`), not a lookalike — cloned by rendering the SAME component a second time, parked at the BEFORE rank (`rank`/`interactive=false` props) with its header twin hidden (`.marelo-slot.is-celebrating{opacity:0}`, so there is never a double image and the header card does not ALSO climb — one event, one thing celebrating it, the mistake the deleted entity toasts made). Four phases (`"out"→"climb"→"hold"→"back"`): "out" is the FLIP itself — a `useLayoutEffect` measures the live header card's `getBoundingClientRect()` before paint (the FLIP's first half), then a DOUBLE `requestAnimationFrame` flips a separate `lifted` boolean true, which is what the card's `transform` (identity vs. centred+scaled) reads. The double rAF is load-bearing, not decorative: a CSS `transition` needs a genuine PAINTED prior value to interpolate FROM, and the clone does not exist in the DOM at all until origin is measured, so painting it already-lifted on its very first frame leaves nothing to animate from -- measured as 100% of the travel landing in the first sampled frame before this was caught. "climb" is where `celebration.to` replaces `celebration.from` and the card's own `useRankClimb` (via `RouteRankCard`'s `rank`/`identity="marelo-celebration"` props) walks from→to — this file owns ONLY the flight; every beat at the centre (digit reel, wing grow, tier burst, flash) is `ui/rankclimb.js` walking `ui/celebrations.js`, the identical registry the star banners run, so "same type of celebration effects" is structural rather than a resemblance. `useRankClimb`'s third argument gained an optional `tune` override for exactly this caller (`{ lane, order, replayKey, tune }`) — read once per climb same as the module-level `tuning()` slot, so a caller can hand it an AMPLIFIED tuning without a second climb implementation; nothing else passes one. "hold" holds the finished rank at the centre; "back" flips `lifted` false again (no double-rAF needed here, since the card already has a real painted "lifted" value to leave from) and acks (`POST /api/marelo/ack`) once `flyBackMs` elapses, or immediately on a click (the card's own `onclick` jumps straight to "back" from any phase — "click to dismiss"). A division-up runs the IDENTICAL sequence at `divisionScale` (a marelotuning.js row, default 0.55) of the flight/hold durations — the ONE named difference between a division-up and a tier-up, not a second component. The shared CSS custom properties (`--fly-ms`, `--fly-ease`, `--backdrop-alpha`, `--backdrop-tint`, `--climb-color`) live on the OUTER wrapper (`.marelo-celebrate`), never the inner card: a custom property only flows DOWN the DOM tree, and the backdrop element reads them too — declaring them on the card alone (an earlier draft's mistake) left the backdrop reading nothing but its own hardcoded fallbacks, silently breaking "the background gradient... should animate from the original coloring to the new coloring" (user, 2026-07-27) the moment a value was actually tuned away from its default. `.marelo-celebrate*` are COMPONENT selectors with no size `@media` at all (`.claude/rules/ui-core.md`'s law) — the card's geometry is measured from the live header at runtime and expressed as a transform, correct at every width by construction. Verified by FRAME-SAMPLING (uilab's `trace.record()`; the `tools/cdp.py` rig it names elsewhere was extracted into uilab 2026-07-28), never by eye: both end states of a transition always look right, which is how a snap survives review indefinitely. The confirmed defect this caught (2026-07-28, fixed same session) is documented above; the working trace reads first-frame displacement ~0.07% of total travel (curve starts at rest) and the final three per-frame deltas strictly decreasing to ~0 (comes to rest), for BOTH directions. Ack semantics, the pref-off no-show ack, and the app-root mount are unchanged from the deleted overlays: `RankUpCelebration` (this file) is celebrate.js's old `RankUpOverlay` under a new name, still gated on `celebrationsEnabled()` |
...
| The route SWAP (picking a different route, never an earned rank-up) | `ui/routeswap.js`'s `useRouteSwap` — a THIRD state alongside "climb" (earned, walks the ladder) and "snap" (today's scope change, instant): an EXCHANGE, the old measurement leaving and the new one arriving, that must never read as an achievement. `useRankClimb`'s `identity` gate is UNCHANGED and still fires the SNAP the instant a route pick lands (the label is part of `identity` in `components/header.js`); this hook is a separate clock layered on top of that snap, keyed on `marelo.scope_id` (never `activeRouteId` directly, for the same "server's own answer to what did I just rate" reason `RouteRankCard`'s `cardLabel` already uses it) so a rank-MODE change — which also flips `identity` — still snaps exactly as before and is untouched by this feature. All four moving parts (icon squash/pop, rank-text crossfade, route-name crossfade, bar lerp) are driven off ONE `progress` value in `ui/components/marelo.js`'s render, which is the "same timing" requirement as a number rather than four durations matched by hand: the bar and both crossfades use `climbcurve.js::barEase` (monotone, never overshoots — a route's rank may legitimately be LOWER than the one just left, and the bar must ease DOWN, not sweep up before correcting); the icon reuses `ui/celebrations.js`'s `CELEBRATIONS.tierAnticipate`/`.tierBurst` `icon` functions DIRECTLY (called with this hook's own progress, split at `tune.swapExchangeAt`, never through a beat or an `on:` list) rather than a second squash implementation — which rank is actually DRAWN swaps at that same exchange point, the flattest frame, exactly where a tier crossing already hides its own cap swap. The two crossfading texts (`marelo.js`'s `rankLabelText`/route label) sit inside `.marelo-swap-fade` (`index.html`, a `display:grid` overlay — both halves in the SAME grid cell via `grid-area:1/1` — so a fade never reflows the card); wrapping them turned `.marelo-bar-body > .context-value` / `.marelo-bar-icon-col > b`'s CHILD combinators into silent no-ops for the whole swap window, the exact `.context-select > select` trap ui-core.md already documents, so both are now descendant selectors. Frame 0 is set SYNCHRONOUSLY inside the hook's own effect rather than left to the first `requestAnimationFrame` tick — without it, `useRankClimb`'s own synchronous snap (an earlier hook in the same component, so it commits first) painted the DESTINATION state alone for one frame before the swap's rAF got a chance to reset it back down to where the animation actually starts from: a flash-then-reset, caught only by frame-sampling (uilab's `trace.record()`), never by an end-state screenshot. Tunables live in `ui/marelotuning.js`'s "Swap" group (`swapMs`, `swapSquash`, `swapExchangeAt` — duration, squash depth, and the exchange point; the widen/burst-floor/overshoot shape is borrowed straight from climbtuning.js's own rows rather than re-exposed here) and are read by `ui/routeswap.js`, a SECOND reader alongside `marelocelebrate.js` in `tests/test_ui_marelotuning.py::READERS` (the two never read each other's rows: one is the FLIGHT for an earned rank-up, the other is the EXCHANGE clock for a route pick). `/ui/tunemarelo.html`'s demo (below) drives it through the card's own real picker, not a scripted playback |
| The practice-log tuning fixture, when the CARD grows a field (2026-08-20) | `ui/tunelog.js` — it hand-builds the section and attempt shapes the real server sends, so a new server-resolved field silently empties whatever the card draws from it. The PB gate did exactly that: with no `pb_action`/`pb_blocked` every actions cell rendered EMPTY (no Save, no Undo, no chip) and with no `pb_by_strat` every card read "no PB · Standard" — on the one page the card is tuned by feel, and no test could go red for it, because the standing rule here is that no test may pin a tuning page's CONTENTS. Each fixture row STATES its `pb_action` as data (a plain success defaults to Save; the PB row says `"undo"`, the foreign row says null with its reason) — never derived, because a derivation here is a second copy of the server's precedence that nothing compares — and one row is tagged a foreign strategy on purpose — since 2026-08-22 that row draws NO action and no delta (his ruling), so it is the emptiest row shape the card is tuned against. `tests/test_ui_rank_progress_track_log_card.py` pins COHERENCE rather than contents (every success row offers an action or prints why not; all three PB-tag shapes appear), which is the shape that stays legal here |
| Tuning the OVERALL rank-up (2026-07-28, second tuning surface) | `ui/marelotuning.js` — eleven rows (Flight/Beats/Backdrop/Intensity groups: `flyOutMs`, `flyBackMs`, `flyEaseIn`, `flyEaseOut`, `centreScale`, `beforeHoldMs`, `holdMs`, `divisionScale`, `backdropOpacity`, `backdropTint`, `tierAmplify`) plus a Swap group (`swapMs`, `swapSquash`, `swapExchangeAt` — see the row above), same exact surface shape as climbtuning.js (`MARELO_TUNABLES`/`MARELO_DEFAULTS`/`MARELO_GROUPS`/`withMareloDefaults`/`mareloTuning`/`setMareloTuning`) so `ui/tunecontrols.js` and `server/tuning_api.py` need no special case for a second registry. The flight curve's cubic-bezier exposes ONLY its two horizontal control points (`flyEaseIn`/`flyEaseOut`) — the vertical ones are pinned at 0 and 1 in code, never tunable, which is what makes "starts from rest" and "comes to rest" structural rather than a value someone could mistune away; guarded by `tests/test_ui_marelotuning.py::test_the_flight_curve_starts_and_ends_at_rest` (asserts no row ends in `y1`/`y2`, not a value). `tierAmplify` multiplies four of the CLIMB's own tunables (`levelFlashTier`, `levelFlashDivision`, `shakeAmplitude`, `burstOvershoot`, the `AMPLIFIED` list in marelocelebrate.js) for this surface only, which is how the overall rank-up outranks a star banner running the identical registry without a second set of effects. **`ui/tunecontrols.js`** is what makes a second inspector safe to add at all: `Control` (one registry row → one number/range pair or a select for a `CHOICES`-shaped row, `defaults` taken as a PROP rather than a module import since the two inspectors read different registries) and `ControlGroups` (the group loop) are extracted from `tune.js`, so the climb inspector and this one cannot render a tunable differently or disagree about when it counts as "changed". `LevelPicker` (the from-rank/to-rank picker) lives here too, and for a sharper reason than DRY: `tune.js` is a served PAGE with its own module-level `render()` call, so importing anything from it — even a pure component — would ALSO execute that render as an import side effect and mount the climb inspector into whatever page did the importing; `tunecontrols.js` has no such side effect. **`/ui/tunemarelo.html` + `ui/tunemarelo.js`** is the inspector: a REPLICA of the header's context grid (same reasoning as `ObjectiveCard` above — the real header is welded into store.js behind a live store) wrapping the REAL `RouteRankCard`, a from/to `LevelPicker` pair, a Play button that fabricates a synthetic `{from, to, key: Date.now()}` celebration and mounts the REAL `MareloCelebration` KEYED on that key (so pressing Play again mid-flight remounts from "out" instead of resuming a stale `phase`), `ControlGroups` over `MARELO_TUNABLES`, and SAVE → `POST /api/tuning/marelo` (same `rewrite_defaults`, same `TUNING_REGISTRIES` map, no Python of its own). `MareloCelebration` is called directly rather than through `RankUpCelebration`, deliberately bypassing the "Celebrate rank-ups" pref gate — a tuning session must always be able to press Play, even with celebrations switched off in the main app's settings drawer (that state only shows as a warning banner here, matching tune.js's own `prefersReducedMotion`/`celebrationsEnabled` warnings, which likewise never block Play). Reachable from the settings drawer (`header.js`, origin-relative `href="/ui/tunemarelo.html"`, beside the climb link) — `tests/test_header_ui.py::test_the_tuning_page_is_reachable_from_the_app_and_from_the_launcher` pins the header half; the launcher script (`run-test-server.bat`) line for it is not yet added, tracked as a follow-up rather than silently left inconsistent |
