...
| The paste door | `ui/components/importpaste.js` — a textarea, a CHECK that previews through `TrackerService.preview_import` (the same planner the button performs), and every unusable line drawn back with its number, its own text and a reason |
| The LiveSplit door | `ui/components/importlivesplit.js` — a file input, previewing on pick. The copy states both rules that would otherwise read as bugs: a gold lands on a segment and never on a star, and a split naming something this database has never heard of is named back |
| The sheet-link door | `ui/components/importlink.js` — a URL field; asks for a runner name ONLY when the workbook turns out to be an Ultimate copy, because every other sheet is entirely yours already and asking would be a question with one answer |
