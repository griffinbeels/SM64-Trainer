...
| A runner's Ultimate Sheet column | the live sheet | stars (IGT); the six seeded Bowser movements, and any segment of yours a Library row is LINKED to (RTA) |
...
What is genuinely still open on the two:
  ...
  * **the sheet's castle-movement rows that nothing links or name-matches.**
    22 of GTM's rows; each lands the moment its target is linked to a
    segment in the Library (or a segment of that name exists), which is the
    mechanism he asked for rather than a per-row picker.
