...
THAT RESIDUAL IS CLOSED (task 0117, 2026-09-04). Walking back OUT of a subarea
on foot zeroes the counter exactly as a retry does and the destination area
cannot separate them — and it stopped appearing 0 times the day he practised
Slip Slidin' Away, whose star sits OUTSIDE the slide: every grab published the
7 s since the cabin door and took Usamune's 49"10 as a correction 1.47 s later
(journal ids 34449/34450), which on screen is *"it takes FOREVER for us to
show the actual final, aggregate time"*. The second signal is the game's own
warp countdown firing (the anchors row above); the correction watch is
unchanged and still runs to `RESULT_SETTLE_FRAMES` as the backstop.
...
## A spawn names its subarea and its spawn point

**Round 20 item 3, 2026-08-08.** `spawned` carries `area` (the settled
gCurrAreaIndex) and `spawn_node` — `sWarpDest`'s nodeId, readable at the
spawn edge because the struct SURVIVES the completed warp (the 2026-08-05
probe's own finding, read for a second purpose; no new memory read), taken
only when the struct's level+area match the spawn's, so a savestate load
degrades to null rather than a foreign node (`spawn.py::spawn_node_from`,
pinned in test_spawn.py). The pyramid's top and bottom entries carry
different nodes — VERIFY live: his next pair of pyramid entries must
journal two values. The label reads "Spawned into Lethal Lava Land:
...
dialogue shows Usamune at `0'11"53` while the practice log recorded `0'11"60`
— journal id 28790, raw `counter` 346, and 346/30 = 11.53 where we published
348. His words: *"Need to make sure we're detecting when the trigger occurs
more tightly."* `score_moment_clock.py --usamune 0'11"53` attributed it to that
row and returned `counter + 0`.

**A GLOBAL FLIP WOULD HAVE BROKEN THE DOOR**, whose own screenshot says +2 and
is untouched — which is why the value moved onto the registry instead of the
constant changing a fourth time, and why the scorer now derives the shipped
offset per kind and returns ONE VERDICT PER KIND. Scored globally those two
true readings read as a contradiction, and acting on that verdict would have
re-broken whichever kind was measured first. A caused kind raises instead of
borrowing the door's number.

**WHY they differ is OPEN and deliberately not guessed at.** The obvious story
— a dialogue stops Usamune's counter — was MEASURED AND IS FALSE: across every
textbox in his journal the counter advances 1:1 with `global_timer` through the
dialogue, **25 samples, 0 frozen frames.**

**HISTORY IS BACKFILLABLE HERE AND WAS NOT DONE** (his call to make): unlike
the pipe family, every moment payload carries its raw `counter`, so a replay
pre-pass could re-derive. Priced before offering it: **33 textbox rows, all
moving by exactly −2, no other kind touched, and exactly ONE definition in the
corpus ending on a textbox (`Wooden Door → 70 Star Door`), with 0 saved PBs.**

**THE DOOR'S OWN NUMBER, kept because everything above rests on it**:
`counter + 2`, one frame ABOVE a live-verified pipe, and the difference is
real. Settled 2026-08-06 by a screenshot holding both numbers in ONE frame
(Usamune 1'06"83, our top row 1'06"80, journal id 2279 on raw `counter` 2003);
deleted the day before on the reading that its 16 samples had come off the
`global_timer` DELTA the practice log was showing, which equals `counter + 1`
on a clean run and so could not tell the two numbers apart. Both fixes were
needed and only one survived that round — `segments.IGT_ARM_SKEW_FRAMES` (make
the segment take the moment's number at all) stands, and this value is restored
beside it.

**The instrument is the deliverable, not the constant**:
`tools/score_moment_clock.py` scores any reading off his screen against the RAW
COUNTER the row carries — never against what we published, which is the number
under suspicion — so moments journaled either side of a flip stay comparable
and ONE screenshot settles the next disagreement instead of another round of
recalled comparisons. Its pure core is pinned by
`tests/test_score_moment_clock.py`, mutations proved. WHY a door differs from a
pipe is still open; the offsets are not.
