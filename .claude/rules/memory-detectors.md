...
| The host present counter hunt (map v4) | `memory/present.py` — `PresentHunter` finds PJ64's frames-presented counter each session by the probe's signature (~30 ticks/s, phase-flip share 0.03–0.6 against gGlobalTimer; `tools/probe_host_present.py` is the evidence, ANSWERED 2026-08-23). HEAP address, moves per session, so it can never be a `layout.py` row and has no sync gate — the hunt IS its verification, and a wrong find degrades to the v2 frame map, never to wrong values (`replay/frameclock.py` refuses implausible runs). Background daemon thread, started by the poller only while frames advance; the poller's watchdog invalidates a counter that froze while the game ran. Sim-clock injectables (`sleep`/`clock`) let `tests/test_present_hunter.py` run the whole hunt on fake time |
