...
| What the PLAYER CAUSED, as opposed to what Mario DID | `tools/probe_objects.py --pool` (the instrument; the detector does not exist yet — round 10). **His framing is the design** (2026-08-07): *"the switch press is the switch's, but that switch press never occurs without mario… by association, it's therefore mario's action"*, and *"We need to record anything that the player can directly cause / trigger / interact with"*. `MOMENTS` reads MARIO's action byte, which is the wrong end of the causal chain for a switch press or an enemy's death — MEASURED: he pounded the blue coin switch in WF and TTC and nothing fired, because a floor switch is pressed by its own behaviour code watching his position and never reaches `MARIO_OBJECT_POINTERS`. The engine writes the association on the OBJECT: `oInteractStatus` (0x134) is what Mario just did to it, `oAction` (0x14C) its own state machine, `oHealth` (0x184), `oInteractType` (0x130) — offsets from the decomp's `include/object_fields.h`, VALIDATED because two rows of that same table (`oPosX` 0x0A0, `oHomeX` 0x164) are ones this project measured independently and match. `--pool` reads the whole pool per frame and logs every change with the behaviour, position and Mario's action. **Nothing is built on it until a real capture exists** — the switch was reasoned about twice and measured once, and only the measurement was right |
