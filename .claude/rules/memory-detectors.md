...
| Star-grab + IGT logic | `detectors/star_grab.py` — docstrings carry the domain rationale; IGT itself comes from the shared `detectors/igt_clock.py` (result→counter→reconstructed) which ALSO stamps key.py's grand star — every displayed time routes through it, never a frame delta. **OPEN, measured but not yet fixed — Usamune's `STOP` setting decides WHEN Usamune stops, and we always read at our own action edge.** Live report 2026-08-01, seven grabs across three TIMER presets: under `STOP=GRAB` we matched his screen exactly (twice, gap 0); under `STOP=GRABX` we were LOWER by 11/28/9 frames; under `STOP=XCAM` by 11/1. **The gap is not constant, so no offset can fix it** — and under GRABX our source was `result`, i.e. Usamune's own written value did NOT hold the number on screen, which kills the hopeful reading that the result store follows STOP for free. Two facts that constrain any fix: `STOP` is not a boolean (**GRABX** is a third value; the presets also move `REALTMR` to FADE/HIDE and `PSSRACE` to RACE), and under XCAM the clock fell through to `counter`, i.e. no result write landed within `RESULT_FRESH_FRAMES` of the touch. Both point at reading too EARLY. Next measurement, already built: `uv run python tools/verify_star_stop.py` watches 8 s AFTER each grab and prints where the counter came to rest and when the result store was written, so the human can say which is on screen. If the counter's rest point is the answer across all three STOP values, the fix needs no settings block at all — the counter stopping IS the setting expressing itself |
