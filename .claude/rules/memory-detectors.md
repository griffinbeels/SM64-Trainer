...
| Death detection | `detectors/death.py` — action-set edge + pending-warp pulse for void-outs (pit falls fire BEFORE level_changed; docstring carries why); closes open attempt as outcome "death". **OPEN, deliberately unfixed**: this is the one time source that does NOT go through `igt_clock.py` — the payload carries `curr.igt_overall` RAW, so a death row sits one frame (3 cs) under every star/key/pipe time, which all add `DISPLAY_TICK`. Nobody has measured whether the display tick is a star-path calibration or a display-wide property, and routing death through the clock on the assumption would re-time every historical death row, stars included (projection stamps a star's death attempt from this same payload). Ruled 2026-08-01: **live-gate it BEFORE fixing it** — `uv run python tools/verify_death_clock.py`, a behavioural gate, not an address one (`USAMUNE_OVERALL` is already sampled, so no `VERIFY` row and `verify_addresses.py` is not the instrument). Its pure core is pinned by `tests/test_verify_death_clock.py`, five mutations proved |
