...
| The host present counter hunt (map v4) | `memory/present.py` — `PresentHunter` finds a per-frame host counter each session by per-tick SHAPE: steps of exactly +1 at ~33 ms median gap, ranked by tick-time wander against gGlobalTimer's edges (locked mirror = the edge series by proxy, accepted when alone). The bands were REBUILT 2026-08-25 after the original rate+phase classifier (and `tools/probe_host_present.py`, whose docstring carries the correction) selected 0xED1F9D4 — a 10 Hz +3 FPS accumulator that quantised the shipped map to −1..−8 slots on the first scored clip; averages cannot see an accumulator, tick shape can. HEAP address, moves per session, so it can never be a `layout.py` row and has no sync gate — the hunt IS its verification, and a wrong find degrades to the v2 frame map, never to wrong values (`replay/frameclock.py` refuses implausible runs). Background daemon thread, started by the poller only while frames advance; the poller's watchdog invalidates a counter that froze while the game ran. Sim-clock injectables (`sleep`/`clock`) let `tests/test_present_hunter.py` run the whole hunt on fake time — its fake world's impostor IS the production one, mutation-proved |
