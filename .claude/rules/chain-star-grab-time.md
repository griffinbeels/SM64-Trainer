...
| 6 | the projector | `Attempt.igt_frames`, with `time_corrections` folded in so every consumer sees one number | `src/sm64_events/tracking/projection.py`, `src/sm64_events/tracking/service.py` | `uv run python tools/measure_target_queue.py` — replays each journal under both revisions and diffs every recorded row's times | re-project a `Connection.backup` copy whose payload you edited | a recorded row whose time moved, or an attempt missing from the after-side | zero differences, because both sides ran the same code (`--before` left at the working tree) — or because the change was at hop 3 or 4: the replay reads the flags the DETECTORS stamped into the journal, so a classification fix scores 0 differences on every journal (2026-09-04, four journals, 0 rows moved) and can only be scored on hand-built snapshots, since no recording of his polls exists |
...
- 2026-09-04 hop 3 — "it takes FOREVER for us to show the actual final, aggregate time… we trigger incorrect resets upon shifting between areas" (Slip Slidin' Away, journal ids 34447-34450): the slide's exit DOOR lands in area 1, the destination rule read it as a retry, the bank was dropped and the row published the 7 s leg with Usamune's 49"10 arriving as a 1.47 s correction — fix at the epoch, never at the gate: the game's own warp countdown FIRING is the second reading, so a door out banks like a chimney in, and the same flag ends the projector's false reset.
