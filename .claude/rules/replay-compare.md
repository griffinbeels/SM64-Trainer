...
| WHEN each game frame happened (the clip's frame map) | `replay/frameclock.py`, TWO series. The edge series: the poller marks every frame edge's wall time (250 Hz, `server/poller.py::_due_for_a_snapshot`). The FEED series (v2, preferred): `recorder._on_frame` tags each captured picture with `latest_frame()` at capture, the tag rides `FfmpegAvSink.submit` to the feeder, and `mark_feed` records what was actually fed — because the edge series plus ANY constant lag measured ±1 game frame of two-sided error on one clip (`tools/score_frame_map.py`, clip 741, 2026-08-23: 8/44 button-changes exact): the picture's presentation and the feeder's staleness each wobble against the logic clock, and tagging at capture + recording at feed removes both. `ReplayService.view()` writes the sidecar's `frame_map` (video frame k -> raw game frame, `DISPLAY_LAG_FRAMES` folded in) at extraction; a save keeps it. Never absorb a per-clip desync into a new constant — score it with the tool; the offset is only the fallback for pre-map clips |
