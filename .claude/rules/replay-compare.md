...
| WHEN each game frame happened (the clip's frame map) | `replay/frameclock.py` — the poller marks every frame edge's wall time (250 Hz precision, `server/poller.py::_due_for_a_snapshot`); `ReplayService.view()` turns it into the sidecar's `frame_map` (video frame k -> raw game frame, `DISPLAY_LAG_FRAMES` folded in) at extraction, and a save keeps it. THE answer to capture jitter (round 32 item 17) — never absorb a per-clip desync into a new constant; the map says it outright and the offset is only the fallback for pre-map clips |
