...
| The **sheet best** for a strategy — the standards table's bottom row | `library/examples.py::sheet_best`, sharing `strategy_entries` with `example_clips` so a strategy's fastest time and its example videos come from the SAME rows (two resolutions of "which sheet rows are this strategy's" is the divergent-duplication class). ONE difference, and it is the point: an entry with **no video still counts**, because the question is what the fastest recorded time IS — a videoless faster run beats a slower filmed one and `video` is simply null. Ties keep the sheet's own answer (`Multiple [3]`) rather than inventing a winner. Served as `sheet_best` on `GET /api/ranks/standards?entity=`, where a **dead video costs the LINK and never the row** — the opposite trade from `cutoff_videos` beside it, where the link IS the payload; stated in that endpoint so the difference reads as a decision. Drawn by `ui/components/standards.js` (`.claude/rules/ui-ranks.md`) |
