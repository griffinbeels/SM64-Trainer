...
| The **Overall Rank Standards** section at the top of a target page | `ui/components/overallstandards.js` reads `/api/ranks/standards?entity=&version=`: authoritative `overall_curve`, separate `overall_overrides`, and `calibration_revision`. `librarymodel.js::overallCurveOf`, `curveBands` and `curveStandingOn` evaluate its full nodes through `ui/timecurve.js`; eight cutoffs cannot reconstruct generated Overall, and no strategy owns its tiers. Only an older payload without `overall_curve` uses the legacy adapter; malformed compiled data fails visibly. The page resolves its local grading entity from its own key, explicit association or name match; an unlinked movement retains the explanation beside its link door. The section opens independently of strategy accordions for comparison. Generated curves show **nine bands, easiest first from Capless to Mario, each with five indented divisions** (45 total), cap art, shared range labels, and the current/next markers. A reader with no saved time sits at Capless V; an unrankable curve stays unranked. The current editor sends seconds to `PUT /api/ranks/overall/{entity}/{rank}?version={version}`; Reset sends `DELETE /api/ranks/overall/{entity}?version={version}`, affecting the displayed US/JP region. Fixed cutoffs are labeled, invalid saves keep an error visible, and reset returns to current generated Overall without editing Strategy. Compatible new observations can move unpinned cutoffs; the basis text distinguishes Sheet population/family evidence, estimates, and an inherited-pin legacy fallback. Revision changes refetch the payload; identity/version switches discard the old editor state. Guards: `tests/test_overall_model_js.py` (full nodes, all 45 attained ranges, malformed payloads), `tests/frontend/overallstandards.test.js` (actual editor, region, error and reset behavior), and `tests/test_ui_overall_refresh.py` (refresh without remount). [Living rank calibration](../../docs/ranking-calibration.md) owns fitting, tuning and activation. |
