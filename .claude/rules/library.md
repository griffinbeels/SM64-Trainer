...
| Serving the [[Rank board]] (every runner + you, one scope, MARELO-descending; one runner's breakdown widened against you) | `library/board.py` (task 3 of spec 2026-08-20-ranked-leaderboard), routed at `/api/leaderboard*` in `server/ranks_api.py` (never `/api/library/*` — this is MARELO's scope surface pointed at the sheet, not a library-browsing route). Consumes `library.ratings.runner_scores`/`runner_times` (Task 2) and never re-grades a time itself — `tests/test_single_source.py`'s "a sheet entry's time graded against a standards ladder" row would catch a second `scoring.best_ladder`/`progress_for_time` call anywhere else in this zone, which is why per-entity tier/division classification here routes through `tracking.marelo.entity_ladders` instead. `RunnerScoreCache` (one instance per running app, held in `create_ranks_router`'s closure) memoizes the {runner: {entity: score}} map on four inputs — library revision, adoptions map, grading version, and a hash of the WHOLE vetted standards store (`ranks_store.to_json()["version"]` is the bundled seed's version and never moves on an edit; see the module docstring). Grading is PB-basis always, whatever `rank_mode` says, and exclusions never apply (`_groups(..., excluded=set())`) — a runner's denominator must not shrink because of the user's own choices. Both routes run through `run_in_threadpool`: scoring 448 runners measures ~29ms for the map, ~46ms to aggregate Overall, both past a 33ms game frame |
