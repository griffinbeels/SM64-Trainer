...
| Fitting a row's rank ladder | `library/ladders.py` model 3 keeps all eight cutoffs and 45 reachable subdivisions. Mario I is an elite near-best target; narrow ranges extend slower by one frame per subdivision. `ranks/timecurve.py` and its JS twin interpolate in frames. The exact fitting/Capless contract is in `docs/sheet-rank-standards.md`. |
