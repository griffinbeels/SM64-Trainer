...
| Writing YOUR times back out as a sheet column (the reverse direction) | `library/export_column.py::column_lines` walks `read_rows` and the payload's `targets` IN LOCKSTEP — a new target opens exactly where a `SheetRow.opens_target` row does, so the two only ever need to agree on order, never on row numbers; `""` wherever nothing maps (a header/spacer row, a row naming nothing in its own target, or the caller's `resolve`/`place` refusing). Shares its two authority-ordered questions (a star approach with a `matched_strategy` resolves directly; anything else goes through the caller's `place(target, item, kind)`) with `server/import_api.py::sheet_row_placer`, so a column can never claim a row the import itself would not land — same door, one direction reading it, the other writing it. `sheet_time` is the sheet's own cell notation (`43.70`, `1:21.00`), round-tripping through `sheet.parse_time` unchanged; not `import_runner.py`'s `m'ss"cc` rejected-row notation. Served by `server/scorecard_api.py::get_column`, off a LIVE fetch (`_fetch_column_source` reads `read_rows` and `library/store.py::build_and_stamp` off the SAME bytes, so the two walks stay in step) — never the cached snapshot, because the row layout has to match the sheet you are about to paste into |
