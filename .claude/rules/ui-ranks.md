...
| WHY one strategy's ladder can BE the Overall one (2026-08-20) | `ui/components/standards.js` draws `.std-overall-note` above the table when a SINGLE strategy owns every cutoff of the entity's best-possible ladder: *"<strat> is the fastest strategy at every rank, so its times are also the Overall standard for this star."* His report, 2026-08-15: *"it also still seems a bit weird that the 'Standard' strategy is the exact progression for the 'Overall' ranking."* It is not weird — the entity ladder is a pointwise minimum, so a strategy fastest everywhere IS that minimum — but nothing said so, which is the correct-but-unexplained shape he reads as a bug. **Not an edge case**: 76 of the seeded entities with 2+ strategies have a sole owner, his own `star:7:2` among them. Read off the payload's `overall_owners` (`scoring.best_ladder_owners`), never re-derived. **Gated on the number of LADDERS, not of ranks** — with one strategy defined it owns everything trivially and the sentence would describe a race with one runner. The panel does NOT re-poll, so the note appears on the next open rather than the instant a cutoff is edited (pre-existing; `tests/test_ui_overall_owner_note.py` reloads rather than asserting a refetch that does not happen). Four guards, mutation-proved |
