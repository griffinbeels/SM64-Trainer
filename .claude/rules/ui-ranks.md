...
| The [[Scorecard]] card + its [[Goal]] picker | `ui/components/scorecard.js` — root `.practice-card.scorecard-card`, SCOPE-DRIVEN since round 6 — the mount (`rankpage.js`) passes the tab's own `scopeId` and the card fetches `GET /api/scorecard?scope=` (refetch on scope change AND `t.mareloRev`, clearing the old scope's rows first); one `ScoreRow` per payload row (the Overall template's courses + Secret, a route's step-groups, a course's own row), each `ScoreTile` painting its own icon (`entityIconSrc`) behind the gap text, and `SumCells` drawing the row's summed **You** and **Goal** as TWO right-hand columns under a `.score-head-row` heading pair (round 7: *"I would expect to see the 'You' and 'Goal' columns always visible... there should be column labels"* — the single gap chip that preceded them kept both numbers in a hover; the gap survives as colour on the You cell, and the card-wide Upstairs RTA foot was deleted the same round, "for now"). Each row also carries a red × (`.score-row-remove`, round 8) that excludes every cell in it through `POST /api/marelo/exclude` — the SAME door the Rank tab's breakdown writes, never a card-only ignore list, so the card and the scope's rating cannot list different things; the calls are SEQUENTIAL because `set_rank_excluded` read-modify-writes one ui_state KV and parallel writes would silently lose all but the last. It is LAST in the DOM because this row is a plain auto-placed grid and DOM order IS column order — placing it second (where it reads more naturally) shifted every later cell one track right and overlapped You with Goal by 2px at 26 viewports; the first attempt at that fix retuned the TRACKS and made it 39, which measuring the actual rects settled. A route row whose cells all sit in one course wears the COURSE name rather than the step's authored label, so the corpus's "WF — 7 stars" count does not reach this card (it stays in the Run tab and the route builder, which want it). Rows are keyed and expanded by INDEX, never by `course_id` — a route scope's rows all carry `course_id: null`, so the old key collapsed every one of them onto "secret" and expanded them together; the guard for it (`test_expanding_one_row_expands_only_that_row`) has to drive a ROUTE scope, since the Overall card cannot exhibit the bug at all (measured: the same test written there passes with the bug fully restored). The `folded` tile concept is DELETED — the 100c cell is combined with its companion star server-side; two `CopyButton`s (Copy sheet column, Copy scorecard CSV) COPY TO CLIPBOARD rather than download — the desktop WebView2 shell's download behaviour is unverified, and a dead download button is the same bug `.claude/rules/acceptance.md` names for a disabled control, so `GET /api/scorecard/export.csv` stays reachable by URL instead. The goal picker's pure logic (`divisionOptions`/`fmtGapCs`/`goalGroups`) lives in `ui/scorecardgoal.js`, NOT this file, and is re-exported here — that file cannot import Preact (no node/browser importmap under `node`, the same reason `rankpage.js` needs source-scan tests instead of a node import), so the node-driven half of a component that DOES import Preact has to live one file over; scorecard.js's own header comment states the deviation from the task brief that produced this split. The Runners half of the goal picker's group list is fetched LAZILY on the picker's first open (`GET /api/library/runners`, 448 names), not on the card's own mount — the card remounts every time the Rank tab does. Root class `.scorecard-card`, with `width: 100%` + `container-type: inline-size` set on it directly — what lets its own `@container` rules (never `@media`, per `.claude/rules/ui-core.md`'s container law) size the tile grid — `tools/contact_sheet.py --scorecard` is the sweep. **Round 2** (`fix(scorecard)`, his report "when I press the dropdown, it goes offscreen"): any row's chevron expands it (`ScoreRow`'s own `expanded` state, LIFTED into `Scorecard` so several rows stay open at once) into `ScoreRowDetail`'s per-star Star/You/Goal/Δ table, the Ultimate Sheet template's own shape; each Goal cell edits in place into `pendingOverrides` (entity_key → goal_cs), recomputed live through `applyGoalOverrides` (`ui/scorecardgoal.js`) with no server round trip per keystroke, until a "N custom times edited" bar's save names and writes them through `PUT /api/scorecard/goal {kind:"custom", ...}` — the SAME KV write a saved name later re-selects from the picker's own new "Custom" group, listed ahead of Divisions. The goal picker itself now opts into `SearchSelect`'s `align="right"` (`ui/components/searchselect.js`, a new prop — every other caller keeps the old left anchor) so its popup anchors under the trigger's right edge instead of running past `.app-main`'s `overflow-x: hidden` clip at every width |
