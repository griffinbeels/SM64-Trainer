...
| To change... | Edit |
|---|---|
| Changing the rank SCOPE while a run is in flight | `store.js::runBlocksScopeChange` (the rule, import-free, node-driven by `tests/test_ui_run_scope.py`) + `pickRoute(id, {confirmed})` returning the `RUN_ACTIVE` sentinel + `header.js::RunScopeWarning` (the dialog). **The route IS the rank scope here** -- one control, two meanings -- so switching it mid-run would silently re-rate that run against a plan it is not following. His ruling 2026-08-03: *"we have to stop the run before changing ranking scopes... You're allowed to change it, just that it will also stop their active run. The dialogue should warn them."* A warning with a way through, never a refusal; Cancel is the safe default. The store REFUSES and returns a sentinel rather than asking anything itself -- a store must not own a dialog, and `window.confirm` would block the event loop while the run it is asking about carries on, cannot be styled, and is invisible to the responsive rig (header.js still uses one for session delete, so the guard test is scoped to the component, not the file). Re-picking the route you are ALREADY running is not a change and must not warn -- the case no render can check, since nothing visible happens either way. The other half, in `runview.js`: **arming a run SETS the scope to that route** (`confirmed: true`, because starting the run IS the decision). Without it the practice page stays on Overall, the route never narrows the selector, and the run's own next split is not auto-selected -- the shape the whole report started from |
...
| Rank UI (badge/banner/table/route medals) | `ui/components/ranks.js` (`RankBanner`, rendering a `RankIcon` inside its own `.rank-icon-slot rank-banner-icon` wrapper — see the Rank icon SPACING row above and the Rank icon STYLE row below for the registry + dispatcher — + `RANK_MODE_OPTIONS`) + `standards.js` (collapsible editable table; columns = store ∪ section strategies, ordered **SLOWEST on the left, FASTEST on the right** by `ui/ladderorder.js::slowestFirst` — the table is a PATH, read bottom-left to top-right as you improve (user, 2026-08-03: *"they would start from the slowest strat and ranking (bottom left)... so as they get better, they'd move from left to right"*; *"This should be for ALL rank standards now and forever"*). A column's key is its Mario cutoff, else its own fastest defined cutoff (a ladder may skip tiers, and dropping such a strategy to the edge would misplace a genuinely quick one), else the far LEFT for a strategy with no times at all — unproven rather than slow, and the left edge is where a run starts. Ties keep arrival order so a new strategy slots in instead of reshuffling its neighbours, and `Infinity - Infinity` is guarded because a comparator returning NaN has undefined results rather than merely wrong ones. On a 100-coin star the sort runs WITHIN each exit-star band and then orders the bands by their own fastest column, so the progression still reads left-to-right without a column leaving its heading. Rendering is pinned separately from the sort (`tests/test_ui_hundred_coin_render.py`) — a correct sort feeding a table that draws in some other order is exactly what the unit test cannot see; BANDED by exit-star variant on a 100-coin star — a `colspan`'d `.std-variant` header row over each variant's columns, with the LEAF in the column head, ordered so a column always sits under its own heading; bands come from the payload's `strategy_groups` and are empty for every ordinary entity, which renders exactly as before (spec 2026-08-03-hundred-coin-exit-variants); each band also wears its OWN palette hue (user, 2026-08-03: *"different colors for the headers for each of the overarching exit star options... to better delineate which columns belong to which overarching exit star strategy"*) — `standards.js::BAND_TINTS` is the single owner of the cycle and hands the stylesheet a `--std-band` custom property per column, so the CSS never names a colour and adding one is a row in that array; six hues, one more than a course can have exit stars, assigned AFTER the bands are ordered so the leftmost band is always the first hue; three weights of the tint (heading wash, fainter wash on the leaf names, and a left rule running the FULL height of the table, since the question is which COLUMNS belong to a heading rather than which heading is which). `--violet` was added to `:root` for it — deliberately not a teal, `#3a8d8d` is `.stdtable .col-active`'s ring. Guarded by `tests/test_ui_hundred_coin_render.py::test_each_exit_star_band_wears_its_own_colour`, which asserts the PAINTED colour for the reason `.claude/rules/ui-core.md`'s verification norms give; every cutoff prints in the Usamune notation `fmtSeconds` owns (`1'21"32`, `23"00` under a minute) rather than raw seconds — user, 2026-08-03: *"that matches the format we actually display in the practice log"*; the EDIT-mode input still takes plain seconds, which is the stored unit and what the strategy modal's ladder already asks for; × is dual-meaning — on a SEEDED strat clears its standards, on a CUSTOM strat DELETES via `?purge=true` with confirm; each cutoff time links to the fastest example video within its tier's band (server-resolved `cutoff_videos`: xcams clips + library entries, task 0098) and each tier row expands into its five subdivision threshold rows — **full detail below: [The standards table's example links and subdivision rows](#the-standards-tables-example-links-and-subdivision-rows-task-0098)**; Edit-mode ▶ per-cell override; the tier-label cell backgrounds from `capGradient(rank) || rankColor(rank)` (Rank icon STYLE row below) — a real base→spots→base gradient for the two patterned tiers rather than a flat fill, since a flat fill is a lie for a cap that's actually two-tone (addendum 2, 2026-07-25; a plain white Toad row was the live complaint); cell text and the strategy-modal ladder are `capName(rank)`, never the raw tier key — the xcams bridge is the one deliberate exception, `title="{capName(rank)} · {rank} on xcams"` (e.g. "Waluigi · Gold on xcams"), so the external site's own vocabulary stays findable; `markerPosition` draws a "you are here" mark BETWEEN two ladder rows — your time is essentially never AT a cutoff, so it is a 0..1 fraction of the gap the current grading basis falls in, interpolated against the ACTIVE strategy's OWN ladder rather than the entity's best-possible one so the bracketed cutoffs can never disagree with the rows on screen; the score shown next to it is never recomputed in JS, it rides `sectionRank.score` from `tracking/views.py`'s `_section_banner`/`ranks/scoring.py::score_for`); wired into practice.js (section banner + attempt medals), progress.js (medal nodes), routes.js (per-step medals + route avg; the practice page's own RouteFocus card was deleted 2026-08-05); banner "avg of N" basis line + mode-aware unranked sentinel — **full detail below: [How full a rank bar is DRAWN (and the one bar that is never anchored)](#how-full-a-rank-bar-is-drawn-and-the-one-bar-that-is-never-anchored)** |
| Practice-log Strategy / Overall rank view | `ui/components/practicelog.js::LogCard` + `ui/rankdisplaymode.js` + `ui/components/ranks.js::RankBanner(swapKey)` + `ui/routeswap.js`. One banner, always: separate measurements get two pressed-state buttons in the existing kicker slot, Strategy on a fresh entity and the saved per-entity preference thereafter; a shared ladder gets only selected Overall and its click is inert. The preference key is the entity, never the strategy, so changing strategy preserves it and choosing on one star/segment does not leak to another. A mode change is an EXCHANGE, never an earned climb -- **and so is a change of the active STRATEGY (2026-08-23)**: `practicelog.js::strategySwapKey(activeStrat, rankMode, hasSeparateRank)` is the one swap key (`strategy:<name>` in the Strategy view, `overall` in the Overall view, null while no strategy is picked), so a strategy swap squashes the cap, pops it back with the new rank and LERPS the bar -- his ruling, "it shouldn't animate the full ranked bar filling up... it incorrectly gives a false sense of progression", reversing 2026-07-27's "repeat the animation process" replay (`replayKey` is gone from rankclimb.js). The FIRST pick still climbs from the floor: neither hook treats null -> key as an exchange. Pinned by `tests/test_ui_strategy_swap_exchange.py` (both directions, mid-swap bar samples; red under the old wiring). It reuses MARELO's exact `useRouteSwap(..., mareloTuning())` clock, pre-paint first frame, cap squash/pop and exchange pivot, sequential `exchangeFade`, `barEase` over DRAWN `barFill` endpoints, and tier-colour interpolation. The kicker/buttons stay fixed while rank name, basis and next-step text exchange. Star ↔ segment parity is structural because both render through `LogCard`. Guarded by `tests/test_ui_practice_log.py`, `tests/test_ui_section_parity.py`, `tests/test_ui_text_exchange.py`, rendered browser checks in `tests/test_fixture_reaches_the_real_page.py` / `tests/test_ui_rank_progress_track_log_card.py`, and the responsive sweep. |
| The **Sheet Best** row at the bottom of the standards table (2026-08-20) | `ui/components/standards.js` renders it after the `ROW_ORDER` map inside the SAME `<tbody>` (real `<tr>`s keep the columns aligned by construction, the reason `StdSubRows` works the way it does), from the payload's own `sheet_best` (`server/ranks_api.py` → `library/examples.py::sheet_best`, `.claude/rules/library.md`). One cell per strategy: the fastest time anybody has recorded on the [[Ultimate Sheet]] for it, the runner under the time, linked where the run was filmed. It exists because the top of a ladder is not the top of the sport — he expanded Mario into its divisions, read Mario 1 and said *"there actually ARE faster times than this"* (2026-08-15). **No cap icon and no rank hue, by his instruction**: it grades nothing, so wearing a rank's art would claim it does; a hairline above the row says "past the top" instead. A strategy the sheet has nothing for gets a dash, never a guess, and the row takes no part in `markerPosition` — it is not on the ladder, so it can never wear the `◀ you` bracket. Named Sheet Best rather than WR because what we know is that it is the fastest row on the sheet |
| WHY one strategy's ladder can BE the Overall one (2026-08-20) | `ui/components/standards.js` draws `.std-overall-note` above the table when a SINGLE strategy owns every cutoff of the entity's best-possible ladder: *"<strat> is the fastest strategy at every rank, so its times are also the Overall standard for this star."* His report, 2026-08-15: *"it also still seems a bit weird that the 'Standard' strategy is the exact progression for the 'Overall' ranking."* It is not weird — the entity ladder is a pointwise minimum, so a strategy fastest everywhere IS that minimum — but nothing said so, which is the correct-but-unexplained shape he reads as a bug. **Not an edge case**: 76 of the seeded entities with 2+ strategies have a sole owner, his own `star:7:2` among them. Read off the payload's `sole_overall_owner` (`scoring.sole_overall_owner`, beside `best_ladder_owners`; resolved server-side since 2026-08-22 so the Library page and this panel cannot disagree), never re-derived — the rule, its LADDER-count gate (one strategy owns everything trivially; a race with one runner) and its tie rule all live and are tested there. The noun ("star"/"segment") comes from `entitysection.js::nounOfKey`, shared with the Library page. The Sheet Best row is its own component, `SheetBestRow`, beside `StdSubRows`. The panel does NOT re-poll, so the note appears on the next open rather than the instant a cutoff is edited (pre-existing; `tests/test_ui_overall_owner_note.py` reloads rather than asserting a refetch that does not happen). Four guards, mutation-proved |
| The `◀ you` marker's PB fallback (fixed 2026-08-20) | `standards.js` positions the marker from `sectionRank.basis` in the averaging modes and from the saved PB in `pb` mode — and that fallback used to read the STRATEGY-BLIND PB while drawing it on whichever column was active, so a time run on 3x LJ planted a "you are here" badge in the Standard column and his own progress read as misattributed (his report, 2026-08-15). `practicelog.js` passes `sec.pb_by_strat` now. A strategy he has never run simply has no marker, which is the honest state and the one the old fallback existed to avoid |
| The standards panel's JP/US **version switch** + JP editor (2026-08-15, tasks 0056/0065) | `ui/components/standards.js` mounts the shared `ui/components/versionswitch.js` (JP left, US right — the SAME control the Library hero mounts; one implementation) right after Community defaults. VISUAL ONLY: `shownVersion` (null = "don't ask" → the server resolves the grading version) rides `GET /api/ranks/standards?version=`, and the panel reads `data.version`, never its own state, so it cannot show a version the table has not fetched. When `data.version !== data.grading_version` the note says "Viewing JP standards · you are graded on US" (explain, never dim). The card passes `gradingVersion` (the view's `game_version.effective`) so an untouched panel refetches when the SETTING flips (mutation-proved). Edit mode reads the payload's EXPLICIT `strategies_us`/`strategies_jp` (never the version-resolved `strategies`), so the switch stays live while editing and the US field can never show a JP-resolved time; per strategy a "JP differs" head checkbox (`.std-jp-toggle`) — disabled, with the reason in its title, only when the JP ladder is NOT in `clearable_jp_strategies` (the server's own list of overlays in his file; a sheet-fitted JP ladder is not his to clear, a typed JP time on it is) — and, for flagged strategies, a stacked US/JP cell (`.stdcell-versions`, video button spanning both rows) whose JP field PUTs `?version=jp`; unticking a clearable strategy confirms then `DELETE …/{strategy}/jp`. The `◀ you` badge's title names the graded version when the shown one differs |
...
| [[Runner page]] — one community runner's ratings, read-only, two doors in | `ui/components/runnerpage.js` (task 5, spec 2026-08-20-ranked-leaderboard) + its doors and the ▶ rows — **full detail below: [The runner page, its doors and the ▶ rows](#the-runner-page-its-doors-and-the--rows)** |
...
`ui/components/scorecard.js` — root `.practice-card.scorecard-card`, SCOPE-DRIVEN since round 6 — the mount (`rankpage.js`) passes the tab's own `scopeId` and the card fetches `GET /api/scorecard?scope=` (refetch on scope change AND `t.mareloRev`, clearing the old scope's rows first); **round 9 (2026-08-28) rebuilt the surface into his reference sheet's own shape** — *"four big columns... Each cell is a card representing a course... compact enough so that if the window was landscape, I could take a nice picture"*: `.score-cards` is a DETERMINISTIC column grid since round 10 (his screenshot report: CSS multi-column's height-balancing staggered the card edges, *"this weird spacing that feels unsettling"*): **round 20 (2026-09-01) fixed the placement to his own grid** — *"the order should be displayed in COURSE order... We should fill columns top to bottom, then left to right... [BOB] [BBH] [DDD] [THI] / [WF] [HMC] [SL] [TTC] / [JRB] [LLL] [WDW] [RR] / [CCM] [SSL] [TTM] [Secrets + Bowser combined into a single card]"*: `scorecardgoal.js::cardColumns` chunks the payload's own order COLUMN-MAJOR into `columnCountFor(pane)` stacks (4 above a 1320px pane, 2 above 560, else 1 — round 11's floors, now picked by the COMPONENT and announced as `data-cols`, because the chunk count and the drawn track count must be one number or reading down a column stops being course order; round 12's six-track 4K shape is RETIRED with it, and `test_the_css_declares_a_track_rule_for_every_count_the_component_picks` fails if a count has no CSS rule). Both live in `ui/scorecardgoal.js` so node drives them; the payload arrives in course order with ONE specials card last (`ranks/scorecard.py::SPECIALS_LABEL`, merged in the BUILDER so the Sigma, the CSV and the ignore button read one row — a display-only merge would need a second `_sum_tiles` in JS). The outer grid stretches every column to one height and a column's `grid-auto-rows: 1fr` equalises its rows (his *"tops and bottoms of each card should end in the same place"* as geometry — guarded by `test_the_cards_sit_in_four_aligned_columns_reading_down_in_course_order`, mutation-proved) (round 11 raised 1080 → 1320 so a 4-up card fits its times AND an unwrapped name; each breakpoint move adds its widths to uilab's matrix and surfaced more samples of the already-owed starholder×starrank overlap — see `known_defects`). **Round 11 made the CARD the grid**: one column system (icon/name/You/Goal/Δ, time columns with XX'XX"XX minimums) that the `.score-labels` row (You/Goal/Δ, round 7's labels ruling restored), every `.score-line`, and the `.score-card-foot` join as SUBGRIDS — a wide time widens the column for the whole card, which closed "especially when the times are longer (e.g., 1'00"00 and above), it causes misalignment"; the `minmax(0,1fr)` lines row is what pins the foot to a stretched card's bottom. Each line's icon+name is one BUTTON (`.score-line-link`, spanning both columns) opening the Library on that entity via the `openLibrary` intent with the tile's server-stamped `strat` and its `goal_cs` — `librarytarget.js`'s `focusGoalCs` arrival lands on the example CLOSEST to the goal (active strategy's approach first), opens its DivisionGroup and blinks the entry; hover turns the name `--blue` and reveals a library glyph that always reserves its slot (visibility, never display, so a wrapped name cannot reflow) — and that glyph rides GLUED to the last word (`.score-line-tail`, `white-space: nowrap`, round 13): reserved inline space put the glyph ALONE on a second line whenever a name filled its column, so six real names drew one line of text inside a two-line box and their icon centred below the visible text (*"it appears to have incorrectly loaded above the course icon"*). Guarded by `test_no_star_name_leaves_a_blank_line_under_itself`, which measures the last TEXT line's bottom against the name box's — driven at 2860px, since nothing wraps at 1500 and the defect is unreachable there. Guards: `test_every_card_labels_its_columns_and_keeps_them_in_register` (subgrid mutation-proved), `test_every_line_is_a_door_to_that_stars_library_page`. **Round 12 (2026-08-29), the full-monitor shape**: the component MEASURES its own pane (`viewport.js::useMeasuredWidth`, lifted out of rankpage.js — importing from rankpage would close a cycle). Its six-track ≥1900px rebucket (*"5 columns of 3 for the main courses, and then the two secret/bowser fights cards as the 6th column"*) is RETIRED by round 20 — there is one specials card now and his grid is four columns wide at any width — but the MEASUREMENT it established stayed, and is now load-bearing rather than an optimisation. What made the width exist at all: `.workspace`'s app-wide `max-width` rose 1500 → 2600 (*"the overall width needs to be way bigger when the screen is full"*); widths above the old cap are unmeasured by the responsive matrix except the scorecard's own 2860px guard — a stretch defect on another tab up there is a plausible next report. The same round made the sidebar's `position: sticky` actually FIRE: `overflow-x: hidden` on html/body/#app made non-scrolling scroll containers (sticky's silent killer) — all three are `overflow-x: clip` now, pinned by `tests/test_ui_sidebar_sticky.py` on the OUTCOME (sidebar top after a hard scroll), mutation-proved. One `ScoreCard` per payload row (course cards + Fights + Secret — all 120 stars plus the fights, his own packing answer), each wearing its course's own THEMED tint (`CARD_TINTS`, one table, BOB grass → RR violet; the one specials card wears the castle gold its stars have always worn) via a `--card-tint` custom property the CSS `color-mix`es into background/border (with a fallback, per ui-core's custom-property rule) plus a real drop shadow, each card a head (course art `.score-card-art` for courses 1-15 + `.score-card-name` + head gap + the ×), one `ScoreLine` grid line per tile — entity icon, full WRAPPING name (`overflow-wrap: anywhere`, round 9's "no word is cut off" by construction), your time wearing your rank's 14px cap (`LineCap` → `RankIcon`), the goal time wearing the goal rank's cap, the signed gap coloured good/bad — and a `CardFoot` restating the sum as **Stage Sum** (courses) / Total (Fights/Secret) — SUM since round 14, because *Stage RTA* names a real thing (a route category in this app's own corpus: the run where you play a whole course start to finish) and this number is the addition of separately practised times; the CSV export keeps the sheet template's wording, since it exists to be pasted back into that sheet. The caps come from the payload's own `you_rank`/`goal_rank` (server-graded; the card never grades), and a `.scorecard-caps-toggle` checkbox under the grid adds every cap — OFF by default since round 10 (*"No rank caps by default"*), persisted per browser (`localStorage sm64.scorecardCaps`). Each card's head carries the red × (`.score-row-remove`, round 8 — a full text reset, so it declares its own `min-height: 0`/`display` per `tests/test_ui_button_flex.py`) that excludes every cell in the card through `POST /api/marelo/exclude` — the SAME door the Rank tab's breakdown writes, never a card-only ignore list, so the card and the scope's rating cannot list different things; the calls are SEQUENTIAL because `set_rank_excluded` read-modify-writes one ui_state KV and parallel writes would silently lose all but the last. Cards wear just the course name (round 8: no "— X stars" count; the corpus's "WF — 7 stars" label stays in the Run tab and the route builder, which want it), keyed by INDEX. The `folded` tile concept is DELETED — the 100c cell is combined with its companion star server-side; one `CopyButton` (Copy sheet column — round 19 removed the CSV one at his ask, *"let's get rid of 'Copy scorecard CSV' button"*, leaving the card one export door) COPIES TO CLIPBOARD rather than downloads — the desktop WebView2 shell's download behaviour is unverified, and a dead download button is the same bug `.claude/rules/acceptance.md` names for a disabled control, so `GET /api/scorecard/export.csv` stays reachable by URL instead. The goal picker's pure logic (`divisionOptions`/`fmtGapCs`/`goalGroups`) lives in `ui/scorecardgoal.js`, NOT this file, and is re-exported here — that file cannot import Preact (no node/browser importmap under `node`, the same reason `rankpage.js` needs source-scan tests instead of a node import), so the node-driven half of a component that DOES import Preact has to live one file over; scorecard.js's own header comment states the deviation from the task brief that produced this split. The Runners half of the goal picker's group list is fetched LAZILY on the picker's first open (`GET /api/library/runners`, 448 names), not on the card's own mount — the card remounts every time the Rank tab does. Root class `.scorecard-card`, with `width: 100%` + `container-type: inline-size` set on it directly — `tools/contact_sheet.py --scorecard` is the sweep. **A line's Goal time edits in place** (round 9 kept round 2's editor and moved it onto the line — `GoalCell` keeps the `.score-detail-goal-btn`/`.score-detail-input` classes the render tests key on): edits land in `pendingOverrides` (entity_key → goal_cs), recomputed live through `applyGoalOverrides` (`ui/scorecardgoal.js`) with no server round trip per keystroke — an edited line's `goal_rank` nulls, matching the server's own `None`, which is what keeps `test_scorecard_goal_override_recompute_agrees` exact — until a "N custom times edited" bar's save names and writes them through `PUT /api/scorecard/goal {kind:"custom", ...}` — the SAME KV write a saved name later re-selects from the picker's own "Custom" group, listed ahead of Divisions. **The goal picker is MULTI-select** (round 14, 2026-08-31): `SearchSelect` grew a `multi` prop — the panel stays OPEN while picking (ten runners cannot cost ten trips through the trigger), each row is a toggle wearing `.is-picked` plus a reserved tick column (so ticking never shifts a label sideways), and the caller receives the whole next selection rather than a delta. `scorecard.js::valuesToGoal` stores ONE pick in its own kind and several as a `multi`, so a single pick has no second shape and nothing needed migrating; `goalToValues` reads any stored goal back as the list. The trigger prints the one goal's name, or "N picked" — followed by a LEGEND of pills (round 15, his ask: *"otherwise, it's very hard to understand which of the options you've selected"*), one per pick in pick order, right-aligned so the row hangs off the picker it explains. Each pill wears a colour from `SOURCE_COLOURS` (ten hues, walked by index and re-used past ten), and every star's name carries a DOT in the colour of the pick that set its goal — *"that clearly tells us that player two is the reason that the goal is that time."* The winner is the server's (`tile.goal_source`, the index recorded where `resolve_multi` compares offers), so the two surfaces cannot disagree; the dot rides in the same nowrap tail as the library glyph, or round 13's blank-line bug returns; and its `title` names the pick, so colour is never the only way to read it. Guarded by `test_the_legend_names_every_pick_and_each_dot_wears_its_pick_colour`, which compares PAINTED colours against the pills and is mutation-proved; a single goal draws neither pills nor dots. Every other `SearchSelect` caller passes nothing and is untouched. The goal picker itself opts into `SearchSelect`'s `align="right"` (`ui/components/searchselect.js`, a new prop — every other caller keeps the old left anchor) so its popup anchors under the trigger's right edge instead of running past `.app-main`'s `overflow-x: hidden` clip at every width

## The runner page, its doors and the ▶ rows

`ui/components/runnerpage.js` (task 5, spec 2026-08-20-ranked-leaderboard) — draws through rankpage.js's own `ScopeChips`/`CoverageStrip`/`Breakdown` (all three exported for it), pointed at `GET /api/leaderboard/runner/{name}` + `…/summary` instead of `/api/marelo*`, never a second implementation. `ScopeChips` grew a `source` prop (default unchanged); `CoverageStrip` grew a `readOnly` prop (default off, so RankPage passes nothing) that drops BOTH the ✎ and the click into `EntityDetail` -- that panel reads the viewer's own attempts, which is nothing to do with the runner; a tile with no click renders `.is-static` so it never looks clickable; `Breakdown` grew a `variant` prop (`"yours"` default unchanged / `"runner"` swaps the last three columns for Rank/Their time/Your time/Gap and drops the Ignore control — one table, one named difference, never a second one). **No cycle**: rankpage.js never imports runnerpage.js — `app.js` owns which of the two mounts for the "Rank" nav slot (`openRunnerName` state, cleared on any navigation away from "Rank" so a stale pick can't survive an ordinary later visit), and hands `RankPage` an `onOpenRunner` callback it threads straight to `leaderboard.js`'s own row click. **The board is `LeaderboardCard` (fourth read, 2026-08-23 — "pretty hidden on the rank screen... it should up top... a dropdown Titled 'Leaderboard'. Closed by default. Animate open / closed, reusing our dropdown system"): its own card between the scope chips and the MARELO card, a head button + `collapsible.js::Disclose`, closed per mount. The CARD owns the fetch (`useBoard`, same scope + `mareloRev` keys) and hands `board`/`error` to `Leaderboard`, because `Disclose` renders its contents only while open and animates to the height it measures at that moment — a board fetching on the click folded open onto a one-line "Loading…" and jumped (measured). `.rank-page .leaderboard-card { align-self: stretch }` because `.practice-card` opts out of stretch and a closed card's content is one line. Every test and sweep story that needs rows opens the card first through its real head (`OPEN_LEADERBOARD` in the two UI test files; the two stories in `tools/uilab_project.py`) (every row but your own; yours stays the dead control it always was, just now for a real reason). The OTHER door is a runner's name inside `librarytarget.js`'s entry rows — ONE component, `RunnerName`, that `ExampleCard` and `PlainEntry` both draw through (ladder reading and Leaderboard mode alike), threaded down from `library.js`'s `openRunner` prop through every layer between `LibraryTarget` and the two row components; never the synthetic "You" row (`entry._isYou`), which has no runner behind it. **The page's own door OUT (round 1, 2026-08-22 — his first read: "click on any of the icons within the Coverage list and jump straight to their Library entry for that specific time"; "click the name of any of the entities and be brought immediately to the library page for that entity, focused specifically on this player's entry")**: a lit coverage tile (`EntityTile`'s `onOpen`, distinct from `onToggle` — no `aria-expanded`, the title names the destination; `CoverageStrip`'s `onOpenEntity`) and an entity's name in the breakdown (`Breakdown`'s `onOpenEntity`, `.rank-entity-link`; his own tab's names were plain text until the fork was put to him and he said yes, 2026-08-23: every own row's name is now a door too, `openLibrary({kind:"target", entity, you:true})` — the intent's `you` flag rides to `LibraryTarget`'s `focusYou`, and the auto-expanded `Section` opens the `DivisionGroup` his `standing` sits in (the ◀ you one; its root wears `.is-you` now), scrolls it to centre and blinks it; a row with no PB still opens the entity's page, landing on the section) -- both doors wear `.textlink`, index.html's ONE look for text that navigates: the palette's `--blue`, underlined AT REST, gold on hover (his second read: "Anything that's a link should be obvious that it's a link"); it is also the button-to-flowing-text reset, so a third text door is one class, not a third reset both fire `openLibrary({kind:"target", entity, runner, timeCs})` — the standards table's own deep-link intent grown a runner key; `librarytarget.js`'s arrival effect finds the approach holding that runner's exact `time_cs` among `visibleEntriesFor` (falling back to their fastest visible time under the other version mode), opens its DivisionGroup and blinks the entry, which now carries `data-runner`/`data-time-cs` on BOTH `ExampleCard` and `PlainEntry` so a videoless row can be found. A tile the runner never ran has no entry to land on and stays `.is-static`. **Third read (2026-08-23):** the entity's art sits INSIDE the name's door button ("The course icon should be clickable as well"); the runner variant's last column is the practice log's own ▶ `icon-button` (`.rank-row-play`, play/chevron, `aria-expanded`) for every row whose graded entry has a `video`, opening a `replay-row` beneath the entry (`.rank-video-row`, capped at 480px) that plays through `librarytarget.js`'s exported `ExampleMedia` — the Library card's own media block lifted out of `ExampleCard`, so the Rank tab and the Library never grow two embed paths; `autoplay` makes the ▶ press the play gesture. And his exclusions now shape every board and runner page (server side, library.md's row), so `Leaderboard` lost its `hasExcluded` prop and the basis note its exclusion caveat. **Fifth read (2026-08-23):** his OWN breakdown rows get the same ▶ where the row's PB attempt (`entity.pb_attempt_id`, new on `/api/marelo`) is in `/api/replay/available` (fetched beside the breakdown, re-read with it since the ring shifts) — it opens the practice log's own `ReplayPlayer` in the same `rank-video-row`, full width (`.rank-video-box.is-replay`); `BreakdownRow` builds one `playButton` for both variants. Segments are EXCLUDED BY DEFAULT outside the Bowser-fight / 100-coin-exit categories (server.md's row), which is why a fresh Rank tab shows Lakitu Skip, LBLJ, MIPS Clip and BitS Entry as inert Include rows — the three Bowser course entries (BitDW/BitFS/BitS Pipe Entry, "No Reds") rank by default, exempt by seed key (sixth read). `app.js::setTab(name, {keepRunner})` keeps the runner page alive across a trip taken through this door, so the Rank tab returns to the runner he left rather than the board. The strip's `max-height: 216px` scroll cap is GONE everywhere (his: "I shouldn't have to scroll") — 117 tiles wrap to 5–8 rows at 1500–850px (contact sheet). Gates: `tests/test_ui_runner_page.py` (both doors land on `.library-arrival[data-runner=<runner>]` inside an open division; unpracticed tiles stay dead; the strip's scrollHeight equals its clientHeight on both pages), mutation-proved by dropping `focusRunner` from the arrival gate and by restoring the cap



## The standards table's example links and subdivision rows (task 0098)

Every cutoff cell links to the fastest example video within its tier's band —
server-resolved `cutoff_videos`, whose clip pool is the vetted xcams clips
PLUS every library entry with a time and a video for the row that grades the
strategy (`library/examples.py`; user overrides still win). Task 0098 lifted
coverage 330→1500 of 3069 cells; the residue is strategies with no sheet row
(802 cells) and bands with no published video (767) — his "(if it exists)"
case, not a join bug.

**Each tier row EXPANDS into its five subdivision threshold rows** (item 3,
single-open like the Library accordion). `StdSubRows` mounts real `<tr>`s in
the SAME table so columns stay aligned by construction, animates every cell's
`.std-sub-clip` height with `disclosurePlan`/`feedTuning`'s own tuned numbers
(cell padding INSIDE the clip, so a closed row is truly 0px rather than a
padded sliver that snaps away on unmount), and unmounts after the close
lands. Brackets and example filing derive from the payload's raw `clips`
through `librarymodel.js::bandsOf` — the Library page's own filing walk, so
this table and that page cannot disagree about where a division starts; a
server-side division resolver was rejected as a third door onto the
division-edge rule.

**The tier label is the expand BUTTON and wears the division-I cap** —
numbered, fully winged, the Library TOC's round-2 ruling applied here (item
2; Mario's sign field wears its M emblem, the registry's own design).
Guarded by `tests/test_ui_standards_subdivisions.py`, mutation-proved three
...
`Breakdown` opens in the SERVED order on every scope -- route order on a
route, scope order otherwise ("Default sort should be Route Order", his
second read 2026-08-22; biggest-gain-first was the off-route default until
then, and stays as the toggle) -- and every row leads with the entity's own
art through `EntityArt`, the ONE place this file resolves an icon (the
coverage tile draws through it too); the breakdown's Score/Gain columns render in POINTS (a
