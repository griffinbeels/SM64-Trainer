...
| Which 100-coin LADDER a finished run is graded against | `tracking/hundred_coin.py::classify` (pure) + the resolver `tracking/service.py::_hundred_coin_strat` injects into `Projector`/`replay` -- **full detail below: [Which 100-coin ladder a run is graded against](#which-100-coin-ladder-a-run-is-graded-against)** |
