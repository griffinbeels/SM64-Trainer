...
| Editable-defaults seed reconcile | `tracking/defaults.py` — `reconcile_defaults(db, seed)` + `resolve_steps` over `data/defaults.seed.json` (bundled via `core/paths.bundled_defaults_seed()`; segments block resolved BEFORE routes so a route candidate's `seed_key` can map to the now-known local `segment_id`); mirrors `ranks/standards.py`'s reconcile — an untouched seeded row (`seed_dirty=0`) refreshes from the seed, a dirtied or user-created (`seed_key IS NULL`) row is left alone, a seed row missing from the db is inserted; an unresolved route-candidate `seed_key` writes `segment_id=-1` (renders via the existing `broken` flag). Every row is validated and applied INDIVIDUALLY and `reconcile_defaults` RETURNS the skipped-row problems — one malformed row costs that row only; `main.py` logs what came back. **A user-DELETED default STAYS deleted (2026-08-10)** — it used to resurrect on the next boot, because a seed row with no db row reads as an insert: *"I have deleted the BitS entry segment multiple times on the Main branch, yet it keeps reappearing."* `delete_segment`/`delete_route` now record the row's SEED KEY through `defaults.remember_deletion` into the `deleted_seed_keys` ui_state KV, and reconcile skips inserting a tombstoned key. The ROW is still really erased — Disable stays a separate affordance and was never an answer here, since a greyed-out leftover is the shape he rejected outright (*"marking them as 'removed' is still worthless. Just completely erase them"*, 2026-08-02). A KV rather than a column for `origin_overrides`' own reason: writing to `segment_defs` flips `seed_dirty` and would freeze the row against every future corpus refresh as a side effect of deleting it. **The tombstone STANDS DOWN when a row with that key exists again** (re-import, hand-restore), so one click is not permanent. Both halves mutation-proved — and the reconcile guard alone stays GREEN while the delete route records nothing, which is why `test_tracker_service.py` drives the real `delete_segment` |
...
## The star owns the hand

**Round 21, 2026-08-08 — superseding round 20's family-take the SAME DAY,
off his first live session on it** (image: his own subsection highlighted
gold where the star should have been). His correction, verbatim: *"The
STAR's practice log should be prioritized... The subsections are still
ongoing and still get entries, but the main star that they're a part of
should be the priority. The EXCEPTION would be if the user manually
selects a subsection... Basically, explicit user choices take priority"*;
and on arrival, *"if I load into LLL, I would expect nothing to be
selected by default (even though the Volcano Entry subsection triggers),
because how do you know for sure I'm going to practice anything? You
don't."*

Family membership is `SegmentDef.parents` containing an entity key —
`projection.target_entity_key(target)` is the one translation,
`Projector._family_member_of` the one check. The rules, all in
`projection.py` (round 20's family ANCHOR and take are DELETED, not
patched):
...
3. **A piece's SUCCESS follows onto its PARENT** (`_follow_target`): the
   current target when it is already one of the piece's parents, else
   parents[0] — item 1's "swap to the correct list... showing a different
...
4. **A PICKED piece survives its parent star's grab** — his call,
   ...
   takes from a picked segment at all. A HOOKED piece (an auto fill)
   still yields to its parent's grab, armed or not (round 20's surviving
   half).

**Measured before landing** (`tools/measure_target_queue.py --before
dba9342`, both journals): the repo journal replays **byte-identical**, and
his own sessions move 27 target readings — detection-held pieces going
neutral or handing to their star, the inversion applied retroactively —
...
Both halves — `clock_start` ("trigger" vs "move") and the rule that a closing
event's own Usamune IGT is the recorded time — moved to
**`.claude/rules/segment-clock.md`** on 2026-08-09, when this file passed its
80,000-character ceiling. That file loads whenever you open
`tracking/segments.py` or `detectors/igt_clock.py`, which is exactly when the
question comes up. Nothing was summarised.
...
lone option is auto-selected (`loneOption`, `ArenaRow`), which writes the
