...
| A time he brought rather than played — a `time_imported` journal row the projector turns into a finished attempt (`_imported_attempt`, handled FIRST in `feed` so it touches no live state) | `.claude/rules/import.md` |
...
| Whether a saved time MEANS what the rank implies · what a row may DO about a PB · which strategy is ACTIVE | `tracking/caveats.py`, `tracking/pbaction.py`, `tracking/activestrat.py` — **full detail in `.claude/rules/pb-strategy.md`** (lifted there 2026-08-22 at this file's size ceiling) |
...
| Run projection wiring | `tracking/projection.py` — `Projector` embeds `RunTracker`, feeds it `(ev, closed)` -- the FULL list, including a row the 100-coin finish HIDES from what `feed()` returns (`hidden_exit_ids`): every seeded 100-coin route step lists all the course's stars and needs every one, so the exit star's physical grab must still credit its step even though it records no row (caught in review 2026-09-01; `.claude/rules/hundred-coin.md`); `finished_runs()`/`active_run_view()`/`run_notices`. Runs re-derive on replay (cache like attempts) |
