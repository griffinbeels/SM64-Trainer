...
| One-command release | `tools/release.py X.Y.Z` — preflight (main + clean tree + `gh` auth) → local merge check → HEAD's green full run on GitHub (`tools/full_run.py`; waits for one in progress, refuses if none) → bump `core/version.py`+pyproject → build `--mode all` → zip+manifest → SHA-256s → `gh release create` with SIX assets. **Build runs before any tag/push** so a broken build aborts cleanly. `--dry-run` builds + checksums only. Use the project `release` skill for the full flow incl. user-facing notes |
