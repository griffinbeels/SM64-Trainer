...
| Self-update orchestrator | `core/updater.py` — `check_for_update` (needs ALL FOUR assets: zip+manifest+both `.sha256`s — no unverified bytes ever applies; also stacks `releases` = notes for EVERY version between installed and offered, newest first, via `release_feed.missed_releases` — best-effort) + `UpdateService` (cached check downloads+verifies the manifest and caches the PLAN; `status()` carries `download_bytes` + `attempt`/`attempts`; `begin_apply` = stage→fetch→apply→restart, RETRIED `APPLY_ATTEMPTS` times with `RETRY_DELAYS_S` backoff before `state` ever becomes `error` — the one real-world failure was a transient CDN timeout mid-download (live log 2026-08-01) that the very next click succeeded through, so the user is told nothing until every attempt is spent, and the popup's "Try again" is simply a second `begin_apply`; a failure AFTER the swap is `_RestartFailed` and is never retried, since the files are already installed. `default_opener` is the shipped `http` and is the ONLY thing putting a timeout (`NET_TIMEOUT_S`) on these requests — neither urlopen nor http.client has one, so a stalled socket used to hang the worker forever, and a hang can never be retried; `startup_maintenance` sweeps backups + deletes the migration bootstrap via `--cleanup-bootstrap`). Inert from source; `SM64_UPDATE_FAKE=1` renders the popup in dev |
...
| One-command release | `tools/release.py X.Y.Z` — preflight (main + clean tree + `gh` auth) → local merge check → HEAD's green browser run on GitHub (`tools/browser_ci.py`; waits for one in progress, refuses if none) → bump `core/version.py`+pyproject → build `--mode all` → zip+manifest → SHA-256s → `gh release create` with SIX assets. **Build runs before any tag/push** so a broken build aborts cleanly. `--dry-run` builds + checksums only. Use the project `release` skill for the full flow incl. user-facing notes |
