...
| The timeline and its clock | `ui/components/inputtimeline.js` draws runs on a padded axis; displayed times subtract the lead. `attemptdrawer.js` supplies the replay clock; `controllerpanel.js` draws input/facing state. The timeline follows the presented picture and seeks verified occurrences. Missing identity shows unavailable input. Unique exact targets across resets may be selected; ambiguous repeats are refused. `replay.js` owns Back/Forward/Start/ArrowDown, with shortcuts scoped to the last interacted or focused player. [The chain](chain-input-timeline-frame.md) owns timing evidence and limitations. |
