...
| Resolving one attempt to its own input | `inputs/track.py` — UTC picks the chunks, the anchor frame and `anchor + rta_frames` trim inside them at BOTH ends — and the end runs THROUGH the first frame whose action is a star grab (`STAR_GRAB_ACTIONS`, within `GRAB_SEARCH_FRAMES` of the close), his rule 2026-08-22: "Once mario is in star grab, none of the players inputs matter, so that's where it should stop": a chunk is ten seconds that overlaps the span, so without the end trim the next five seconds of capture draw as part of the run (his first live attempt, 2026-08-22: 598 frames for a 444-frame run) |
