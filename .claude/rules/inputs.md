...
| A PICTURE-FEED CLIP HAS NO GRID -- every frame's own time is the clock | `ui/frame.js::clipClock` builds `{fps, start, times}` from the view (`frame_times` when the clip came off the picture feed, item 38, 2026-09-02); `slotAtTime`/`timeOfSlot` take that clock -- the last frame begun at or before the time, a seek into the frame's own span -- and `inputtimeline.js` (`clock` prop, `mappedFrameAtTime`/`mappedTimeAtFrame`), `attemptdrawer.js` (builds it once per view) and `replay.js` (steps through it) all go through them; no caller may write k / fps. The view carries `frame_times`, `encode` and `feed_match` (`replay/service.py`); the map for such a clip is read off the feed log (`replay/feedmap.py`) and audited by the pad reader with the log's own repeats as its picture flags. Pinned: `tests/test_ui_frame_step.py` (VFR slots and seeks), `tests/test_ui_input_clock.py` (a long hold is one slot), `tests/test_replay_picture_feed.py` (the whole feed with real ffmpeg) |
...
(migration v33). Not guessed from the blob's length: a v1 chunk and a v2 chunk
...
Main sync (2026-09-06): input migrations now occupy v31-v34. `_migrate`
recognizes the pre-sync v27-v30 input schema by `input_chunks`, atomically adds
main's missing import migrations, and advances the version over the already
applied input prefix. Tests in `test_storage_input_branch_upgrade.py` cover all
four old versions, both branches, preserved bytes and rollback/retry. Merely
renumbering the input migrations fails reopening the existing worktree database.
