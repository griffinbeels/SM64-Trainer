...
int main(int argc,char **argv){
 ...
 control_start(argv[1]);control_rom(TRUE, FALSE);puts("ready");fflush(stdout);
 char line[160];
 while(fgets(line,sizeof line,stdin)){
  if(!strncmp(line,"quit",4))break;
  if(!strncmp(line,"romclose",8)){control_rom(FALSE, FALSE);puts("ok");}
  else if(!strncmp(line,"romopen",7)){control_rom(TRUE, FALSE);puts("ok");}
