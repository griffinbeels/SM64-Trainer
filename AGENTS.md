...
| **Imported times** — a personal best he brought rather than played: five doors (by hand, an Ultimate Sheet column, a pasted block, a LiveSplit file, a link to his own sheet) onto one back room | `tracking/importing.py`, `tracking/import_names.py`, `tracking/livesplit.py`, `library/import_runner.py`, `library/sheet_link.py`, `server/import_api.py`, `ui/components/addtime.js`, `ui/components/import*.js` | `.claude/rules/import.md` |
