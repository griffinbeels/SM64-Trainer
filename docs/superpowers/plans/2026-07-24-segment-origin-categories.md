...
- UI components import Preact through the index.html **importmap** under BARE specifiers (`import { h } from "preact"`, `"preact/hooks"`, `"htm"`, then `const html = htm.bind(h)`). There is no `vendor/preact.js` — the files are `preact.module.js`/`hooks.module.js`/`htm.module.js`. Follow the convention in `components/modal.js` (found during Task 4; the plan's original snippet was wrong).
