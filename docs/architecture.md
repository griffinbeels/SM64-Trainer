...
Latest reading, 2026-09-06 for automatic Scorecard goals: **46,281 lines,
...
| `tracking/views.py` (1,917L), `tracking/segments.py` (3,489L), `tracking/service.py` (1,856L), `ui/components/practicelog.js` (956L) | Deep by the ratio, but too large to hold in one context. | Size, not leakage — a different problem with a different fix. Split along existing seams when one next resists a change. **Re-measured 2026-08-06, after the practice-log branch merged: 28,420 lines, 689 publics, 41.2 lines per public name, 41 pass-through (6.0%). The ratio held while the tree grew, so that branch added surface in proportion to what it added rather than in forwarding. One file changed places: `practice.js` fell off this row (1,471L → under the threshold) when the practice log split out of it, and `practicelog.js` took its place at 956L — the one file the branch pushed past what fits in one head. Its natural seam is `LogCard`, which is already a self-contained component inside it. Re-measured 2026-08-06 after the debug-report merge: 28,597 lines, 693 publics, 41.3 lines per public name, 42 pass-through (6.1%) — the one new forwarder is `PerfMonitor.log_path`, the deliberate public door the debug report reads. **Re-measured 2026-08-09 on the subsection-tracking merge: 32,355 lines, 739 publics, 43.8 lines per public name, 44 pass-through (6.0%).** The tree grew 13% and the pass-through SHARE did not move at all — the branch added machinery, not forwarding. Two files it pushed past the threshold, both named here so the next change to them knows: `tracking/segments.py` 3,489L → 4,019L (the matcher plus the piece clock; its seam is the CLOSE path, which is already the only part with its own rule file, `.claude/rules/segment-clock.md`) and `ui/components/stagebanner.js` at 1,017L (four independent row components in one file; the seam is one row per file, and the shared cell already left in round 22 as `celltoggles.js`). Neither is leakage and neither is proposed as work — they are size, recorded so the figure moves rather than the feeling. **Re-measured 2026-08-09 on the sheet-library merge: 34,622 lines, 817 publics, 42.4 lines per public name, 44 pass-through (5.4%).** The pass-through COUNT did not move at all across a 2,267-line branch — 78 new publics, none of them a forwarder — so the share fell for the first time in four readings. One file joins the oversized row: `ui/components/librarytarget.js` at 1,054L (the Library's target page: the link strip, the approach sections, the pieces list and the runner cards in one file; its seam is the section renderer, which already takes its whole shape from one `library-section` contract). `server/library_api.py` reads deepest-per-public in the whole tree (185L behind 1 public name) because it is a router factory — the shape this measure is least able to read, not a finding. **Re-measured 2026-08-10 after the library-search merge: 35,246 lines, 833 publics, 42.3 lines per public name, 44 pass-through (5.3%).** Read that delta as the TREE's and not one branch's — main took several branches between the two readings and the search round is ~235 lines of it. The figure worth keeping is the one that has not moved in five readings: 44 forwarders, now across 35k lines instead of 28k, which is the whole point of measuring depth rather than size.** **Re-measured 2026-08-11 on the reds-as-subsection merge: 35,586 lines, 833 publics, 42.7 lines per public name, 44 pass-through (5.3%).** The branch added 340 lines and **not one public name** — a sixth reading with the forwarder count pinned at 44. That is what a branch looks like when it deletes as much structure as it adds: `celltoggles.js` went entirely, `PracticeCell` lost a prop, and the two new per-kind fields (`Moment.open_states`, `gated_display_lag`) are dataclass fields on a registry rather than new surface. **One file LEFT the oversized row for the first time in this table's history**: `ui/components/stagebanner.js` 1,017L → under 800, because the toggle overlay and the hand-written `RedsCell` came out of it — the seam that row named a reading ago ("one row per file") turned out to be reachable by deletion instead of by splitting. `ui/components/practicelog.js` moved the other way, 956L → 1,033L, taking the nesting, the widened auto-open slot and the retired reds/pipe exclusivity; its seam is still `LogCard`. **Re-measured 2026-08-11 on the entrance-touch round-2 merge: 35,745 lines, 834 publics, 42.9 lines per public name, 44 pass-through (5.3%).** A SEVENTH reading with the forwarder count pinned at 44. The one new public is `segments.speaks_through_a_parent`, a predicate the projector asks instead of inlining a string test — not a forwarder, and the alternative was the same test written at its call site. `detectors/warp.py` is now the deepest file in the tree by ratio at **219.5 lines behind 2 public names** (439L, up from 200L when this table first singled it out for the same reason): four rounds of live measurement have gone into its docstring, and its caller still learns one class and one method. That is what the ratio is for — the file grew 120% while what anyone must know about it did not change at all. **Re-measured 2026-08-15 on the ukikipedia-links merge: 36,284 lines, 838 publics, 43.3 lines per public name, 44 pass-through (5.3%).** An EIGHTH reading with the forwarder count pinned at 44. The branch's two new publics are `links.ukikipedia_url` and `links.ukikipedia_page` (the resolver, not a forwarder -- `star_links` now calls it, which is one door closing over another rather than a new one), and `ukikipedia_titles.py` is 211 lines behind ZERO public names by the tool's count: a generated constant table, which is the shape this measure reads as pure depth. `ui/components/librarytarget.js` stands at 1,262L; the wiki mark added eleven of those. **Re-measured 2026-08-15 on the game-version merge: 36,676 lines, 858 publics, 42.7 lines per public name, 44 pass-through (5.1%).** A NINTH reading with the forwarder count pinned at 44 across a 20-public branch: `core/modes.py`'s setting + resolver, `server/mode_api.py`, the store's `grading_version` property and `clearable_jp_strategies`, `ranks_api.absorb_after_regrade`, `librarymodel.matchedStanding` and the shared `VersionSwitch` -- each a decision behind a name, none forwarding to another. `librarytarget.js` stayed at 1,262L because the branch moved as much out (the per-section chip, the standing rule into `librarymodel.js`) as it threaded in. **Re-measured 2026-08-15 on the version-sync merge: 39,890 lines, 943 publics, 42.3 lines per public name, 51 pass-through (5.4%).** The forwarder count moved for the first time in ten readings, and both additions are named here because both are deliberate: `sync/runner.py::GateContext`'s four doors (`prompt`, `say`, `sleep`, `now`) forward to injected callables so a test scripts a clock and a human's answers through the same surface every gate reads -- the identity-only seam, not leakage; and `memory/objects.py`'s three module-level names (`pool_slot`, `slot_address`, `describe`) forward to a US-bound `ObjectPool` so every pre-existing tool and test keeps its exact call shape (the non-regression rule of that branch). The three can leave once every caller constructs its own `ObjectPool`; that is a follow-up, not a finding. One file joins the oversized row: `sync/address_gates.py` at 851L (0 publics by the tool's count -- a registration module) holds 20 gates plus their polling helpers; its seams are the warp-block trio and the two Usamune hunts, each already a self-contained function group. **Re-measured 2026-08-23 on the rank-strategy-clarity merge: 40,313 lines, 960 publics, 42.0 lines per public name, 51 pass-through (5.3%).** Seventeen new publics and the forwarder count did not move: `tracking/activestrat.py` (`ActiveStrats` and its three identity doors, `reds_pipe_segments`, `masked_strat` -- the last now fenced to that file by `test_single_source.py`), `tracking/pbaction.py`'s three, `scoring.sole_overall_owner`, `TrackerService.pb_action`, `examples.strategy_entries`/`sheet_best`, `SheetBestRow`, `strategySwapKey`, `nounOfKey`. `ActiveStrats.for_attempt` dispatches on the attempt's kind rather than forwarding, which is why the tool does not count it. The branch also RETIRED surface -- `is_current_pb`, `replayKey`, `active_strat_for`, the `PB_GATES`/`gateChip` vocabulary -- so the ratio fell slightly while the tree grew 1%. `tracking/views.py` is 2,169L: this branch took ~170 lines OUT of it (the six restated active-strategy readings, the two PB-row lambdas, the dead `pbs` parameter) and the rest of the growth is other merges since the 1,917L reading. `ui/components/practicelog.js` 1,033L -> 1,107L, still seamed at `LogCard`. **Re-measured 2026-08-23 on the ranked-leaderboard merge: 40,950 lines, 976 publics, 42.0 lines per public name, 51 pass-through (5.2%).** Sixteen new publics and the forwarder count did not move across a 4,851-line branch: `ratings.rate_runners`/`best_entries` (one pass, a dataclass out), `board.RatedSheet` (three readings as methods -- the ghost is a fourth method when he wants it) and `RatingsCache`, `marelo.classify_entity` (the one door both his breakdown and a runner's grade through), `scopes.default_excluded`/`effective_excluded`/`ranks_by_default`, `board.you_pb_by_entity`, and on the JS side `ExampleMedia` (the ONE sheet-video player: ladder cards, the condensed leaderboard rows, the runner page's ▶ and the Rank tab all play through it), `LeaderboardCard`, `RunnerPage`, `EntityArt`, `RunnerName`. Each is a decision behind a name; none forwards. `ui/components/librarytarget.js` grew again, 1,262L -> 1,478L (ExampleMedia lifted out of ExampleCard but staying in-file, the condensed LeaderboardRow, RunnerName, two arrival focuses); its seam is now plainly ExampleMedia + the entry-row family, which could leave as one module the next time this file resists a change. **Re-measured 2026-08-28 on the lint-gate merge: 40,950 lines, 976 publics, 42.0 lines per public name, 51 pass-through (5.2%) — byte-identical to the reading above, because that branch added 1,144 lines and not one of them to `src/`.** It is guard, tool, test, doc and config only. Recorded rather than skipped so the series has no gap: a reading that does not move is evidence too, and this is the shape it takes when a branch adds a mechanism around the tree instead of inside it. **Re-measured 2026-08-31 on the library runner-search change: 40,950 lines, 976 publics, 42.0 lines per public name, 51 pass-through (5.2%) -- byte-identical for a THIRD reading, because that change touched no Python at all.** Its whole surface is one module-private predicate (`hasRunnerMatch` in `librarytarget.js`, deliberately not exported: the section and the page are its only two callers and they must not be able to disagree), so it added no public name and no forwarder. It did push the one file this table has been watching further out: `ui/components/librarytarget.js` 1,478L -> 1,530L. The seam named a reading ago is unchanged and now more clearly right -- `ExampleMedia` plus the entry-row family as one module -- and this change is a reason to take it: the query rule now runs at three nested levels of the same file (section, band, subdivision) and reading it whole is what makes that legible. Not proposed as work; recorded so the next change to this file knows. **Re-measured 2026-09-01 on the strategy-pick fix (task 0113): 41,005 lines, 978 publics, 41.9 lines per public name, 51 pass-through (5.2%).** Two new publics and the forwarder count did not move: `Database.retag_attempt` (one UPDATE behind a name, the door that replaced a full replay) and `Projector.override_strat` (records the reclassification so the live map equals a replay's -- a write into state, not a hop to another call). On the JS side `coalesce.request` now RETURNS a promise where it returned nothing, which is deeper surface at the same width: the caller learns one more thing the same call already does. No file crossed the oversized line; `tracking/service.py` (2,114L), `projection.py` (2,309L) and `storage/db.py` (1,428L) each took a dozen lines of docstring saying why the retag no longer replays. **Re-measured 2026-09-02 on the 100-coin merge (task 0110): 41,194 lines, 978 publics, 42.1 lines per public name, 51 pass-through (5.2%).** The branch added 189 lines and NOT ONE public name -- the forwarder count and the public count are both pinned. Its three new engine names (`_hundred_coin_proof_arm`, `_proven_hundred_coin_exit`, `_hundred_coin_waypoint`) are private to the matcher; its two new memory facts (`MARIO_NUM_COINS_OFF`, `HUD_COINS_OFF`) and `GameSnapshot.coins` are registry rows and a field, not surface; `SegmentEngine._close` grew one keyword (`span_known`) rather than a second close path. `tracking/segments.py` 4,019L -> 4,229L across the merges since that figure was taken (this branch is ~110 of them, all in the 100-coin block beside the arm phase); `projection.py` 2,309L -> 2,339L, the hide-the-exit-row block. Neither is a new seam: the 100-coin family already has its own rule file, which is where a split would start. **Re-measured 2026-09-03 on the subarea-selector + piece-attribution wrap (task 0122 and its follow-on): 41,224 lines, 982 publics, 42.0 lines per public name, 51 pass-through (5.2%).** A FIFTH consecutive reading with the forwarder count pinned at 51, across a branch that DELETED a feature and added a rule. The four new publics are all in `tracking/segments.py` and none forwards: `entity_parents` (which parents can hold a target -- the Python half of `subsections.js::isPiece`, now compared by `test_cross_language_parity.py`), `fires_identically` (the four-field comparison lifted OUT of `lint.py`'s duplicate rule so the author-time warning and the run-time attribution rule ask one question), `attributable_parent` (the rule itself), and `SegmentEngine.definitions()` -- a list copy of engine-private state, the same identity-only shape `armed_ids()` already had beside it. The removal half took surface out at the same time: `COURSE_SUBAREA_STARS`, the `subarea_stars` vocab key, `stage.settling` and `stage.on_the_star_select` all went, along with the detector's whole load-tail window, which is why 30 net lines carry two features' worth of change. `tracking/segments.py` 4,229L -> 4,301L; its seam is unchanged (the CLOSE path, `.claude/rules/segment-clock.md`). No file crossed the oversized line. **Re-measured 2026-09-03 on the test-suite order-and-load wrap: 41,224 lines, 982 publics, 42.0 lines per public name, 51 pass-through (5.2%) -- byte-identical to the reading above, a SIXTH consecutive pinned forwarder count, because that work added 572 lines and not one of them to `src/`.** Collection hook, test-runner door, a new load meter, their tests and their docs: the same shape as the lint-gate merge, a mechanism built AROUND the tree rather than inside it. Recorded rather than skipped so the series has no gap. **Re-measured 2026-09-04 on the subarea-exit epoch fix (task 0117): 41,321 lines, 982 publics, 42.1 lines per public name, 51 pass-through (5.2%).** A SEVENTH consecutive pinned forwarder count and the public count did not move either: 97 lines in, every one of them behind names that already existed. `detectors/counter_epoch.py` took a second reading for the same question (`_warp_fired`, private, plus one recency field and two module constants) and its one public class kept its four-method surface; `memory/addresses.py` gained three registry rows; the rest is docstring in `anchors.py`, which is why that file reads 507L behind 2 publics in the DEEPEST table -- four rounds of live measurement in one docstring, the same shape `warp.py` has. No file crossed the oversized line. **Re-measured 2026-09-05 on the pb-import merge (tasks 0090 and 0105, 103 commits): 45,215 lines, 1,035 publics, 43.7 lines per public name, 52 pass-through (5.0%).** The branch added 53 public names and NOT ONE of them forwards -- no module it touched appears in the pass-through table at all, and the share fell 5.2% -> 5.0% across a 3,894-line merge. (The count moved 51 -> 52 for the tree, not for this branch: main took other merges between the two readings.) Its new surface is decisions behind names: `library/sheet.py::entry_version` (which ROM a sheet entry belongs to -- one door the import and the runner goal both read, so they cannot disagree), `ratings.best_entries(strict=)`, `views.latest_pbs_by_strategy`/`fastest_current_pbs` (the PB slot, keyed on strategy AND ROM), `library/adoptions.library_ladders`, `store.build_and_stamp`/`absorb`, `ranks/scorecard.py`'s builder and `hundred_coin_companion`, `export_column.column_cells`. One file joins the oversized row: `server/scorecard_api.py` at 969L, holding the goal KV, four resolvers (division, runner, custom, multi), the column job and the sheet-style store. Its seam is plain -- the four goal resolvers are already one recursive shape (`resolve_goal` calls itself for a `multi` source), so they leave as one module the next time this file resists a change. `tracking/views.py` 2,169L -> 2,257L and `storage/db.py` 1,428L -> 1,591L across the merges since those figures. Not proposed as work; recorded so the figure moves rather than the feeling. |
