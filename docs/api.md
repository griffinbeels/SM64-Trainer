...
| `DELETE /api/import/{source}` | Erase every imported time one source brought (`manual`, or `sheet:<runner>`). The rows are really deleted, and whatever each superseded becomes current again — the same restoration `/api/pb/undo` performs for one row. Returns `{removed}` |
