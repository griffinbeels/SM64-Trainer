...
| `target_pending` | `pending` (the held intent, or `null`) | **Broadcast-only — never journaled.** A practice target picked for somewhere the player is not standing is HELD rather than applied (`tracking/pending_target.py`); this fires when one is held, committed, dropped by walking into a different practicable stage, or cancelled. Neither the appear nor the drop half fires `target_changed`, so a client that ignores this shows a stale chip until something else refetches. The payload is the same shape `GET /api/session` carries as `pending_target`. An intent has no historical value and a replay must not resurrect one, hence never journaled — the same discipline `stage_changed` follows. |
...
| `POST /api/target` `{course_id, star_id, strat_tag?}` **or** `{kind: "segment", segment_id, strat_tag?}` | Set the practice target. Star targets: legacy shape works (`kind` defaults to `"star"`). Segment targets: `kind: "segment"` + `segment_id`. `strat_tag` distinguishes absent from explicit `null` (Pydantic's `model_fields_set`): key **omitted** leaves the entity's existing strategy untouched; key present and `null` (picking "(no strategy)" in the picker) explicitly clears it, journaled via the same `strat_set null` path as `POST /api/strat` — symmetric for stars and segments. A segment with a truthy `default_strat` can't actually reach "no strategy": a null `strat_set` falls back to the def's own default instead of clearing (`tracking/projection.py` caveat 17), so this only clears on the 10 legacy tricks and user-created segments. A truthy `strat_tag` sets it, as before. 404 if the segment id is not in the definition list. **The target does not always move**: a pick the player cannot practice where they are standing is HELD as an intent instead (`tracking/pending_target.py`) — it commits when they enter that entity's own stage, and is dropped if they enter a different practicable stage first (Castle Inside and the hubs are transit, since every course is reached through them). The response carries `pending: true|false` so a client never has to infer which happened; a held intent appears on the session view as `pending_target` and broadcasts the (never-journaled) `target_pending` event. |
| `DELETE /api/target/pending` | Abandon a held practice intent without moving the current target (the pending chip's × in `ui/components/stagebanner.js`). Idempotent — clearing nothing is `{ok: true}`, not an error. Broadcasts `target_pending` with `{pending: null}`. |
