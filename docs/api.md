...
| `PUT /api/segments/{id}` `{name?, enabled?, start_triggers?, end_triggers?, guards?, waypoints?, category?, match_mode?, parent?, clock_start?}` | Partial update (merged with the stored definition before validation). `waypoints: []`, `category: null` and `parent: null` are explicit clears, distinct from omitting the key entirely (`exclude_unset`). The recorder's re-record save is this endpoint with the full definition — replace in place, so routes/PBs/attempts keep their reference. Triggers re-projection. Disabled definitions stay targetable. |
