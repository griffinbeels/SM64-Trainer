...
| `GET /api/scorecard/column` | The reverse of `POST /api/import/sheet` — your PBs written out as the Ultimate Sheet's own column, one line per live worksheet row, ready to paste back into the sheet. Always a live fetch (503 with a reason if the sheet is unreachable, same wording as the import door). An approach on a star prints only where its vetted `matched_strategy` names which strategy it is; a subsection, a castle-movement approach, or a Bowser row prints only where the SAME facts `POST /api/import/sheet` lands it on vouch for it (an explicit Library link, an unasked name match, or a Bowser row's seed key) — everything else is `""`, never a guessed strategy. A line whose row names an explicit ROM version and whose PB was set on the other one is also `""`. Returns `{lines: [...], sheet_revision, mapped, total_rows}` |
