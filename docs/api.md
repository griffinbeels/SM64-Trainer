...
| `GET /api/library/target/{index}` | One target in full: its approaches and subsections, each with every recorded entry (runner, time, video) and its fitted ladder where the row has enough times. Each row also carries `row_key` (the stable name the adopt endpoints speak) and `adopted` (the entity key you assigned it to, else null); the body carries `adoptable` (false on an instance that mounts no adoptions store, so a client knows not to offer the link door). The entity door below decorates identically. The body also carries `matched_segment` (round 6): for an entity-less target, the user's own segment whose NAME normalizes equal to the target's label (`{entity, name}`, else `null`) -- computed per request against the live definition list, so a segment built mid-session pairs on the next page load; an explicit adoption on a row outranks it. The body also carries `ukikipedia` (2026-08-15): the Ukikipedia RTA Guide URL for this target — its identity's page (a star's own page, a 100-coin star's `<Course> 100 Coins`, a Bowser course/fight's page), else the page its sheet label names (LBLJ, Lakitu Skip, MIPS, a Toad detour), else `null`. Resolved server-side by `links.py::ukikipedia_url` against the wiki's own page list (`ukikipedia_titles.py`, refreshed by `tools/scrape_ukikipedia.py`), so a client never composes a title and a link is always a page that exists. **404** past the end. |
