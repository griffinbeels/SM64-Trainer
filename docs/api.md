...
| `GET /api/scorecard` | The **scorecard**: 15 course rows (7 tiles — stars 0-5 + the 100-coin star) plus one Secret row (PSS ×2, Secret Aquarium, the BitDW/BitFS pipe movements, BitS Reds, Wing, Vanish, Metal, WMotR), each tile `{key, label, clock, you_cs, goal_cs, delta_cs, folded}` and each row a fold-aware `sum` (`{you_cs, goal_cs, delta_cs, counted, total}`, only tiles with both sides AND not folded count toward the sum); `total` runs the same sum over the whole card. `you_cs` is your strategy-blind current PB (igt for a star, rta for a movement); `goal_cs` resolves the persisted goal (below) per tile, `None` on a tile the goal can't grade (no ladder, or a division no integer centisecond reaches). `folded` marks a course's exit star for its 100c PB's variant — the star both sums skip so a Σ never mixes two different exit-star runs. Also returns `goal` (the raw persisted KV, below, or `null`), `goal_coverage: {covered, tiles}` — tiles with a goal time over every tile on the card — and `custom_goals: [name, ...]`, every SAVED custom goal's name (independent of which one, if any, is active — the picker's own "Custom" group). A `kind: "division"` goal grades every tile with a ladder that reaches that division; a `kind: "runner"` goal grades only the tiles that runner has a sheet time for (`library/ratings.py::runner_times` — absent, never zero, so `goal_coverage.covered` can legitimately land short of `tiles`); a `kind: "custom"` goal grades exactly the entities its own saved `times` map names, filtered to keys the card still carries, and needs no ladder or sheet lookup at all. On an instance with a database but no live standards (`service.ranks` is `None`) the card still answers 200, with an empty goal map — a `custom` goal still resolves there, since it needs no ladder lookup at all. On a genuinely **broadcast-only** instance (no database at all — `service.db` is `None`, e.g. a second instance that lost the db lock) every route here, including this one, answers `503` rather than crashing |
