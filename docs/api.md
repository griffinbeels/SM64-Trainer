...
| `GET /api/segments/synthesize?start_id=<id>&end_id=<id>` | Turn two picked `GET /api/segments/timeline` row ids into the `(start_clause, end_clause)` pair a new segment would be defined by, plus a suggested `name` and a plain-English `start_sentence`/`end_sentence` for each end — the hinge behind "record what I just did" (`tracking/synthesize.py`, Task 12, wired up for the timeline picker in Task 13). Looks the ids up directly in the journal, the same source `/segments/timeline` reads. 404 when either id names no journal event. 409 when the pair can't become a segment, naming which problem: picking the **same event** for both ends (it would arm and close on the identical tick), or a row whose type carries no synthesis rule for the role it was picked for (e.g. a `practice_reset`/`state_loaded` row — its position lives in live match state, not its own payload, so it can never become a trigger clause on its own). Sentences render through the same `card_label`/`card_template` machinery the practice card's "waiting for" line uses, so a synthesized-but-unsaved clause reads in the same voice a saved one would. Read-only: no journal entry, no state change. 503 in degraded mode. |
