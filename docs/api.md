...
| `GET /api/scorecard` | The **scorecard**: 15 course rows (7 tiles — stars 0-5 + the 100-coin star) plus one Secret row (PSS ×2, Secret Aquarium, the BitDW/BitFS pipe movements, BitS Reds, Wing, Vanish, Metal, WMotR), each tile `{key, label, clock, you_cs, goal_cs, delta_cs, folded}` and each row a fold-aware `sum` (`{you_cs, goal_cs, delta_cs, counted, total}`, only tiles with both sides AND not folded count toward the sum); `total` runs the same sum over the whole card. `you_cs` is your strategy-blind current PB (igt for a star, rta for a movement); `goal_cs` resolves the persisted goal (below) per tile, `None` on a tile the goal can't grade (no ladder, or a division no integer centisecond reaches). `folded` marks a course's exit star for its 100c PB's variant — the star both sums skip so a Σ never mixes two different exit-star runs. Also returns `goal` (the raw persisted KV, below, or `null`) and `goal_coverage: {covered, tiles}` — tiles with a goal time over every tile on the card. A `kind: "division"` goal grades every tile with a ladder that reaches that division; a `kind: "runner"` goal grades only the tiles that runner has a sheet time for (`library/ratings.py::runner_times` — absent, never zero, so `goal_coverage.covered` can legitimately land short of `tiles`). On a broadcast-only instance (no live session) the card still answers, with an empty goal map |
| `PUT /api/scorecard/goal` `{kind: "division", tier, division} \| {kind: "runner", runner} \| null` | Set (or clear, an empty/absent body) the one persisted scorecard goal — server-side (`ui_state` KV `scorecard_goal`) so the browser and the desktop GUI agree. `kind: "division"` needs `tier` in the rank ladder minus `"Iron"` and `division` in `"V"`-`"I"` (422 otherwise); `kind: "runner"` needs a non-empty `runner` name, graded against the CURRENT Ultimate Sheet snapshot on the next `GET`. No broadcast — the goal picker is the only writer, and the card refetches like the rest of the Rank tab. Returns `{goal}`, the value just stored |
