...
- `POST /api/attempts/{id}/replay` — cut (or reuse) the attempt's clip → `{clip_url, duration_s, truncated, fps, game_fps, source, anchor_offset_s, saved_path}` (fps = encoded rate; game_fps = 30 fps SM64 logic, the frame-step unit; `source` is `buffer` or `saved`; `saved_path` non-null whenever a saved file exists; `anchor_offset_s` is where in the clip the anchor frame's PICTURE is on screen — the pre-pad measured from the clip's own recorded first frame (`start_utc` in the sidecar), plus `replay/service.py::DISPLAY_LAG_FRAMES` (one game frame: the screen shows the frame the game finished a frame ago, measured 2026-08-22) — so the input timeline's frame 0 is at that second of the video; sidecars from before 2026-08-22 have no `start_utc` and it falls back to the current pre-pad setting). `frame_map` (or null) is the clip's **frame map** — entry k names the raw game frame video frame k shows, null before the frame clock's coverage — built at extraction from the picture ledger (`replay/ledger.py`: capture stamped every distinct picture with its composition time and RAM frame, so the rows become the map directly; the clip sidecar keeps its slice as `picture_ledger`, extra stamps included) or, for clips from before it, from the best series the frame clock holds (`replay/frameclock.py`): the hunted present counter's ticks first, the capture-tagged feed series next, the poller's frame-edge stamps last, display lag included; `video_start_s` is the clip's own first video timestamp (an accurate cut leaves a sub-frame remainder on the first picture; every slot the browser computes counts from it, 2026-09-01); `frame_times` (or null) is every video frame's own timestamp in seconds for a clip cut from the **picture feed** (`encode: "picture_feed"`, item 38, 2026-09-02): the clip is VFR, one frame per distinct captured picture, and every slot the browser computes goes through these (`ui/frame.js::clipClock`) rather than k / fps; `feed_match` (or null) says how the feed log matched that clip's frames -- `{frames, matched, repeats, unmatched, bias_ms, residual_ms: {median, max}}` -- and its map's `frame_map_source` is `feed_log`; `encode` is `cfr` for every other clip; `frame_map_mode` names WHICH map shipped and `frame_map_degraded` whether the capture was healthy (item 89, 2026-09-02): `feed_log+offset` is the recorder’s own bookkeeping moved by ONE whole-clip integer the display voted for — what a clip whose feed log covers it (`feed_coverage` ≥ 0.85) gets, because 50-77% of a clip’s frames repeat their predecessor’s pad and 78-94% of the per-slot aligner’s moves sit between two frames the display cannot tell apart; `reader_aligned` means the log did not cover the clip and the display repaired it slot by slot, which is worth 10 points there but is patching capture damage, so the timeline says so. `frame_map_source` (or null) names which answered — `ledger`, `presents`, `feeds`, `edges`, or `mixed` — and after it the map is held to the clip's own PICTURES (one answer per distinct picture, consecutive pictures = consecutive frames) and anchored against the stick display's digits where readable (`frame_map_aligned`/`frame_map_offset`/`frame_map_fit` in the sidecar, plus `frame_map_windows` — [slot_lo, slot_hi, offset, fit, margin] per stretch when the per-picture path found the lag stepping mid-clip; `frame_map_learned` when a display-off clip inherited the median anchor other clips measured). Since 2026-09-01 the **pad reader** (`replay/padread.py`) runs before that anchor and supersedes it when it answers: it reads the input display's six glyph cells out of every video frame and pins the map to what it read; the sidecar then carries `frame_map_read: true` and the view carries `pad_reading` (or null) — `{sure, agree, nowhere, known_cells, slots, anchors, frames_total, frames_checked, frames_agree, unpinned_longest, icons_checked, icons_agree, icons_learned, disagreements}` (`anchors`: white-flash slot -> [reset frame, frame chosen]; `frames_*` the same verdict in GAME frames -- the chip's unit; `unpinned_longest` the longest run of frames the display cannot tell apart; `icons_*` the button-icon channel), where `sure` is the video frames a whole row of the display read, `agree` those whose pad the map matches exactly, `nowhere` reads matching no frame within ±10 (a misread), and `disagreements` is `[slot, row, screen_reads, map_says]` per contradicted frame (first 200). Since 2026-09-04 the **timer reader** (`replay/timerread.py`) runs first on a clip whose picture ledger carries the `igt_overall` stamp: it joins Usamune's on-screen clock to the `(frame, igt_overall)` pair of the same picture and names each picture's frame directly (`frame_map_source` `timer`, `frame_map_mode` `timer` or `timer+bridge`); the view then carries `timer_reading` (or null) -- `{read, legal, pinned, monotonic, backwards, transitions, mechanical, bridged, rejected, display_lag_frames: {min, median, max}}`, where `mechanical` is the slots the clock named outright, `bridged` those carried from the feed log's bookkeeping shifted to the nearest named picture (frozen clock, unreadable, unpaired), `rejected` the readings the join refused, and `display_lag_frames` the measured per-picture gap between RAM and screen -- plus `frame_map_inferred` (true when any slot is bridged; distinct from `frame_map_degraded`, which is capture health). On such a clip `pad_reading` is the display's AUDIT of the timer map, scored before any alignment, and cannot move it. A clip the **capture layer** recorded (`frame_map_source` `plugin`, 2026-09-04/05) carries the plugin's own stamps as its map -- joined with no display lag, `null` on a slot whose present saw two display lists -- and `picture_igt` (or null): the IGT the game held when it drew each slot's picture, which the input timeline's inspector shows as that frame's time. Consumers use the map first and fall back to `anchor_offset_s` arithmetic for clips cut before it existed (round 32 item 17: the capture duplicates and skips single frames, so no offset is right on every frame). Falls back to the saved file when the buffer can't produce the clip; clips saved before 2026-06-12 lack a metadata sidecar → `duration_s` null, `truncated` false
