...
| `GET /api/library/target/{index}` | One target in full: its approaches and subsections, each with every recorded entry (runner, time, video) and its fitted ladder where the row has enough times. Each row also carries `row_key` (the stable name the adopt endpoints speak) and `adopted` (the entity key you assigned it to, else null); the body carries `adoptable` (false on an instance that mounts no adoptions store, so a client knows not to offer the link door). The entity door below decorates identically. **404** past the end. |
