...
| `GET /api/segments/timeline?limit=200&view=steps\|all` | The recent journal, as rows a human can point at to define a segment from what they just did (`tracking/eventlabel.py::label_event`, Task 10). Response `{rows: [{id, frame, type, label, wall_time_utc}]}`, oldest first (newest last) — ordered by the journal's own `id`, **never** `frame`: `frame` is the raw game-frame counter and is not chronological (it runs backward across every practice reset and session boundary — measured against the real journal, 2026-07-28: 469 backward jumps). `view=steps` (default) shows the ~10% of `label_event`'s 9 labellable types a human recognises as "a step I just performed" (`level_changed`, `star_collected`, `warp_entered`, `key_grabbed`); `view=all` adds the 5 that are real segment-boundary triggers too (`attempt_anchor`/`area_enter`/`spawned`/`reset_game`, `tracking/segments.py::TRIGGERS`) but dominate the journal by raw volume or carry no place (`practice_reset`, `area_changed`, `spawned`, `state_loaded`, `game_reset`) — showing them by default would bury the actionable rows under thousands of bare "Reset the level" entries. `limit` caps at 500 (422 above it) and is applied to the most recent rows in the selected view. 422 on an unrecognised `view`. 503 in degraded mode. |
