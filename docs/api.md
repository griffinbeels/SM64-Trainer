...
|---|---|
| `GET /api/attempts/{id}/inputs` | The attempt's track for drawing: `runs` as a list of `{start, length, buttons, stick_x, stick_y, yaw, speed}` on the capture axis (zero-based; a counter restart mid-track is laid end to end, a capture hole stays a hole), plus `frames`, `attempt_frames` (the attempt's OWN length -- `igt_frames` -- which the header prints; `frames` is the drawn span and carries a clip's buffers), `fps`, `target`, `strategy`, `lead_frames` (the **lead-in**: how many of the track's leading frames precede the attempt's own frame 0 — real play the timeline draws as negative frames behind a shaded band, so the attempt's own start and its `igt_frames` length are unmoved). Two optional query params, `from_frame` and `to_frame`, name the range of game frames the attempt's CLIP shows: the track then spans exactly that, buffers included, so the timeline matches the video it sits beside. They may only WIDEN the attempt, never cut it; without them the track is the attempt alone, and the active `template` as `{id, name, origin, runs, actions}` — the same shape as the attempt's own track, Mario included (or `null`; an unloadable template carries `error` and empty lists). `actions` is a separate list of `{start, length, action, label, group}` spans — Mario's own state lasts across dozens of pad changes, so putting it on every run would repeat one fact hundreds of times. It also sends `buttons` (the bit-to-name table) and `angle_units`, so a browser never names a button bit and never does angle arithmetic — no second copy of either to drift. `stretches` is the axis's seams — `[axis_start, raw_start, length]` per ascending stretch of the frame counter, split at a console reset — so a clip's `frame_map` (raw game frames) lands on the zero-based axis in the browser without a second copy of the restart rule. `markers` is the journal's **moments** inside the attempt, joined onto the same axis: `[{frame, type, label}]`, where `label` is the segment recorder's own sentence for that row (`tracking/eventlabel.py`) and membership is the recorder's own default-view rule (`is_step`) — a pole grabbed, a bob-omb picked up, a switch pressed, the star itself. Nothing new is captured; a moment in a capture hole sits at its true axis position, and one outside the track is dropped |
