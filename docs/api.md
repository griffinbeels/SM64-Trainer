...
| `GET /api/scorecard` `?scope=overall` | Build the requested scope from globally saved selections. `goal` is a single rank or a multi with the rank first, then players/custom sets. Automatic includes the current scope-derived tier/division only in this response. Each tile uses the fastest source, with `goal_source` indexing the normalized sources; `player_goal_cs` records the selected players’ best offer before rank/custom comparison for import parity. `strat` is the canonical Library strategy used for the rank target and navigation, resolved from the active name or the entry’s default row. Rank standards follow the grading ROM; player times follow `regions`. `custom_goals` lists saved set names. No choice or celebration state is written by this read. |
| `PUT /api/scorecard/goal` `{kind: "division", tier, division} \| {kind: "automatic"} \| {kind: "runner", runner} \| {kind: "custom", name, times?} \| {kind: "multi", sources} \| null` | Save selections globally. Exactly one rank is always present: the last division/automatic source wins; no rank means Automatic. `null` or empty sources restores Automatic alone. A division must be finite (Iron V is refused). Players and custom sets remain independent sources; nested multi is refused. Automatic stores no calculated tier and resolves from the requested scope on every GET. A custom source with `times` patches its named set with positive, display-snapped centiseconds, preserving entries from other scopes; without times it selects an existing set (404 if absent). The whole request is validated before either KV is written. Returns `{goal}`, the stored single/multi shape; a lone automatic choice is stored as null. |
...
is a positive integer of up to 16 digits. Older or repeated edits from that
client return current state without overwriting it. Expired tokens return 409;
...
- `GET  /api/replay/compression` — which saved replays are being shrunk right now, for the close warning and the recording panel (polled; no WebSocket event). `{active, jobs: [{attempt_id, label, time_text, stage, fraction, from_bytes, to_bytes}]}`: this session's jobs newest first (at most 8, finished ones included); `stage` is `waiting` / `compressing` / `checking` / `in_use` (proven, waiting for the player to stop watching it) / `done` / `kept` (stays full size); `fraction` is one forward-only 0..1 across compressing and checking, null while waiting. `active` is true only for waiting/compressing/checking: an `in_use` replay's work is finished and closing the app swaps it. Compression off answers `{active: false, jobs: []}`; with replay off the route is not mounted, so a client treats any non-200 as idle. The replay is already saved at full size whatever this says; an unfinished job runs again at the next launch.
...
Marking a PB remains a user command. When replay capture is available,
`POST /api/pb` also preserves its video off the event loop and includes
`replay_save: {status: "saved", path, truncated}` or
`{status: "failed", message}`. Media failure does not undo the valid PB time;
the row offers Retry save replay through the existing replay-save endpoint.
Undoing or superseding a PB does not delete its preserved video.

Temporary ring segments, extracted clips, live partial files and the identity
index share accounting. HTTP range reads, extraction and saves lease their
files against eviction. `GET /api/replay/status` includes `storage_pressure`
(recording paused because eviction cannot restore the free-space reserve),
`save_failures` (bounded attempt-id/message map) and `recovery_failures`.
Saved media is outside eviction. True session switches and shutdown discard
unsaved media after active extraction/save operations finish; practice resets,
pauses and closing a drawer do not. See [replay storage](replay-storage.md).
...
|---|---|---|---|
| `GET` | `/api/ranks/standards` | `?entity=<key>&version=us\|jp` | One entity: `entity`, `clock`, resolved `version`, `grading_version`, `strategies` in seconds, `strategies_us`, `strategies_jp`, `jp_strategies`, `clearable_jp_strategies`, `fitted_strategies`, and `estimated_strategies`. Omitted version uses the grading region; invalid version is 400. `overall` contains derived tier cutoffs in seconds; **`overall_curve` is authoritative** (`schema_version`, `interpolation`, full `nodes` in displayed centiseconds/score, `ladder_cs`, `metadata`). `overall_overrides` contains personal pins in seconds for this region; `calibration_revision` identifies effective standards. Strategy edits do not redefine generated Overall. Compatibility fields `overall_owners` / `sole_overall_owner` are `{}` / `null` for generated curves and only describe an exact unpinned legacy envelope. Examples remain `videos`, `cutoff_videos`, `clips`, `library_urls`, `sheet_best`, and `user_videos`; clips use centiseconds, dead community video links are filtered, and `sheet_best` retains a time without a live video. `seeded` and `xcams_url` retain provenance. `strategy_groups`, `exit_variants`, and `exit_star_options` describe 100-coin endings; use the returned qualified names. Omit `entity` for the persisted standards document. See [living rank calibration](ranking-calibration.md). |
...
| `POST` | `/api/ranks/standards/{entity}/reset` | — | Reset Strategy edits to current fitted/seed defaults; preserve Overall pins. |
| `PUT` | `/api/ranks/overall/{entity}/{rank}` | `{"seconds": N}`, `?version=us\|jp` (default us) | Set a personal Overall pin independently of Strategy. Invalid/conflicting pins are 409; unavailable standards are 503. Returns `{ok, calibration_revision}` and broadcasts `rank_standards_changed`. |
| `DELETE` | `/api/ranks/overall/{entity}` | `?version=us\|jp` (optional) | Reset that region, or both when omitted, to the latest generated Overall curve. Preserves Strategy. Returns `{ok, calibration_revision}` and broadcasts `rank_standards_changed`. |
...
| `GET` | `/api/marelo` | `?scope=<id>` (optional) | `{scope_id, label, calibration_revision, marelo, mastery, coverage, tier, division, next_division_at, division_progress, n, practiced, entities:[{key,label,score,tier,division,gain,excluded}], celebration}` for one scope (defaults to the active route, else Overall). `404` for an unknown scope. A held scope `celebration` includes its own `calibration_revision`; an earned rise keeps reporting until acknowledged. The first read after a calibration change absorbs the regrade silently, even before its refresh callback; a stale pinned read cannot change current watermarks or announce a celebration. Per-entity rank-ups remain the live banner's responsibility. |
...
| `GET` | `/api/marelo/history` | `?scope=<id>` (optional) | `{scope_id, calibration_revision, basis, points:[{utc,marelo,tier,division,practiced}]}`. `basis` states that saved performances use current standards; current route membership also applies. Original saved ROM/clock context chooses each curve. |
...
| `POST` | `/api/marelo/ack` | `{"scope": "<id>", "key": N, "calibration_revision": "<revision>"}` | Acknowledge the celebration the UI actually showed, using that celebration's revision. A stale revision returns `{ok:true}` without changing the watermark. Legacy clients may omit the revision; their key cannot exceed the current scope grade. Validation and mutation share the calibration publication lock; the broadcast happens after it is released. A missing `scope` or obsolete `{entity, key}` body is `400`. |

`marelo_changed` is broadcast-only (no journal entry). API GETs pin a calibration
and expose `X-Rank-Calibration`; rank payloads carry `calibration_revision`.
The effective revision includes user edits and grading region. Regrading updates
watermarks without an earned celebration. See the
[calibration contract](ranking-calibration.md#compiled-curve-and-refresh-contract).
...
removes its point. Average modes keep separate entity/strategy/ROM/clock windows;
older attempts without ROM context use the selected grading region.
...
| `GET /api/library/status` | `{sheet_revision, calibration_revision, data_revision, fetched_at, targets, runners, ladder_model, source}`. `source` is `bundled` or `local`; `data_revision` fingerprints observations and `calibration_revision` identifies the generated bundle. `ladder_model` describes Strategy fitting. |
...
| `POST /api/library/refresh` | Fetch the Sheet, prepare Strategy and Overall with resolved assignments, save the prepared snapshot, then publish once. Newer data and same-date corrections can activate; identical content/configuration or older data return `{applied: false, reason}`. Results include `sheet_revision`, `fetched_revision`, `data_revision`, and `calibration_revision`. Failed preparation/write retains the active generation and snapshot. Runs in a worker thread; the full app also refreshes at startup when configured. Source failures return 503. No separate publication step is required. |
