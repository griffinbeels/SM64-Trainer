...
| `warp_entered` | `level, area, action, to, igt_frames, igt, igt_source, released_by, held_frames` | The ENTRANCE TOUCH: the frame Mario collides with a painting, portal, hole or pipe, detected as an edge into a warp-entry action on the already-sampled `mario_action`. The community-comparable timing moment, and since 2026-08-04 the end condition for the 55 definitions that used to end on the course LOAD — measured over 140 castle entries, the `level_changed` that follows is a constant 77 frames away for a painting/portal (range 76-77) and 23 for a pipe. `to` (added 2026-08-04) is the level Mario ended up in, or `null` when the warp relocated him inside his own area (an in-level teleporter), the fade was aborted, or the player paused a pipe's countdown and menu-warped away. **A painting or portal publishes on the touch frame itself** — the game writes `sWarpDest` at the touch (live-measured 2026-08-05, 13 of 13 castle entries; a rule here claimed the opposite for a day, from decomp instead of RAM). A pipe's write lands 20 frames later, so its touch is HELD and published back-dated: `frame` and the `igt` trio are always the TOUCH's, never the release's. `released_by` + `held_frames` (added 2026-08-07, inert telemetry) say which rule published and how long it held: `destination` / `level` / `area` / `reset` / `pause` (the countdown froze — the player menu'd out of a pipe; released at 26 frames instead of the 240-frame cap that cost 8-second rows on 2026-08-07) / `cap`. Rows written before 2026-08-04 carry no `to` at all; `projection.warp_destinations` recovers it on replay from the level edge that followed. The `igt` trio (added 2026-07-31, live report: BitDW "No Reds" read 0'35"90 where Usamune showed 0'35"96) is Usamune's own clock at the touch, from the shared `detectors/igt_clock.py` exactly as `star_collected`/`key_grabbed` carry it, so a segment ending here records Usamune's number instead of a `global_timer` delta; `igt_source` is always `"counter"` at a pipe (Usamune writes its result store on a star grab only). |
...
| `spawned` | `level, kind, area, spawn_node, igt_frames, igt, igt_source` | Mario gained control at a spawn-in. `kind: "intro"` = leaving `ACT_INTRO_CUTSCENE` (file-select spawn on Castle Grounds — Lakitu Skip start anchor, control begins when the cutscene ends); `kind: "spawn"` = edge into a SPAWN_* action (non-intro spawn-ins). `area` (added 2026-08-08, round 20 item 3) is the settled `gCurrAreaIndex`, so a subarea spawn — an L-reset inside the volcano, the pyramid — reads "Spawned into Lethal Lava Land: Volcano" instead of claiming the course started; `spawn_node` is WHICH spawn point placed Mario, read off `sWarpDest`'s nodeId (the game routes every spawn through that struct and it survives the completed warp), taken only when the struct's own level+area match the spawn's — `null` on a savestate load, a stale struct, and every historical row. The pyramid's top and bottom entries carry different node values, so a segment can start on one specific entry. Carries the `igt` trio (`igt_frames`/`igt`/`igt_source`) since 2026-08-06, from the shared `detectors/igt_clock.py` like every other displayed time, so the segment recorder can show a number on every row it draws; rows journaled before that read null and cannot be backfilled. |
...
| `target_changed` | `kind, course_id, star_id, strat_tag, queued` **or** `kind, segment_id, segment_name, queued` | Practice target moved (auto-follows last valid grab, set by command, hooked/promoted by the target queue, or moved/CLEARED by the projector). Same kind-aware shape as `target_set`. May clear to no target — `kind:"star"` with `course_id:null` — when Mario leaves a star's course or a segment arms (active-star/segment exclusivity); consumers that highlight the active target must handle the null-course case. `queued` (round 19) lists the detections waiting behind the held target, in hook order: `[{segment_id, segment_name}]`, empty for most states. |
...
| `time_imported` | `source, course_id, star_id, segment_id, strat_tag, timer_mode, frames, game_version` | A time the player brought rather than played — one row per time an import landed. The projector turns it into a finished attempt (`success`, no anchor, started = ended = this row's wall time, `timed_by: "imported"`), so it is a real practice-log row with every row affordance and no replay; `attempt_completed` follows as for any closure. Its PB row links to this journal id. Erasing the import deletes these rows (`DELETE /api/import/{source}`) |
...
**Personal bests are per (target, strategy)** (2026-08-20). Each section carries BOTH: `pb` is the entity's best saved time regardless of how it was run, and `pb_by_strat` is the ACTIVE strategy's own, in the same `{igt, rta}` shape, `null` on a clock the strategy holds no time for (and `{igt: null, rta: null}` when no strategy is selected at all). A consumer showing one number beside a strategy name wants `pb_by_strat`; one answering "what is the best this target has ever gone" wants `pb`. Mixing them is what made a Standard time read as a 3x LJ PB. An attempt row's `pb_delta_frames` is against the active strategy's PB and is `null` on every row that is not the active strategy's (2026-08-22) — a time on another strategy has nothing to be compared with. Every row also carries `other_strat` (bool): it belongs to a strategy other than the active one (an untagged row included; always false when no strategy is selected), decided by the same gate that withholds its PB button — the practice log dims such rows.

Each attempt row carries the ACTION its PB button may offer, resolved server-side: `pb_action` is `"save"`, `"undo"` or `null`, and when it is null `pb_blocked` is either `null` (nothing to offer — a failure, a cleared row, an attempt with no target) or `{reason, strat}`. Reasons: `grab_timed` (the TIME is not a legal quantity — a caveat key, drawn from the same vocabulary as `caveat`), `foreign_strat` (this run was not tagged with the strategy being practised; `strat` names the one that would accept it), `no_active_strat` (no strategy selected). Strategy is stated before quantity (2026-08-22): a row on another strategy reports `foreign_strat` even when its time is also illegal, because the browser draws nothing at all for such a row; retag it and the quantity reason surfaces. **Do not re-derive this from `strat_tag`** — `POST /api/pb` refuses on exactly this field, and a second derivation is how a button starts offering what the server rejects. `"undo"` is also the only statement that a row owns its OWN STRATEGY's current PB, so more than one row per section can carry it; to mark the row behind the entity-wide `pb`, match `pb.attempt_id` (the `is_current_pb` flag that used to say this was retired 2026-08-22 after the Rank tab misread it).
...
**`fitted` — every graded rank names whether its ladder is Ultimate-Sheet-
derived.** Each star/segment section's `rank` (`_section_banner`) and
`entity_rank`, `rank_by_star[...]`, `segment_targets[].rank`, an attempt row's
own `rank` (`_attempt_json`, and the matching progress-graph dot), a route
step's `rank`, and `GET /api/target/ranks`' per-entity map all carry a plain
`fitted` bool beside `rank`/`division` — `ranks/standards.py::is_fitted`'s
own contract: `true` when the ladder graded here is Ultimate-Sheet-derived
...
Segment definitions with both `start_triggers: []` and `end_triggers: []`
are manual entries: they support imported times and standards but never arm
from game events. A definition with only one empty end is invalid. Automatic
Sheet pieces carry their parent in `parents`; edits and deletions survive refresh.

All endpoints are under `/api`. JSON in, JSON out.

| Endpoint | Description |
|---|---|
| `GET /api/session?clock=igt\|rta[&scope=session\|lifetime]` | Full session view: target, attempts per star, stat chips, PBs, catalog; `scope=session` (default) shows only the active session, `scope=lifetime` aggregates all sessions; star sections are ordered newest-activity-first, the target's section is always present (pinned active star), and each section carries markers_by_strat (per-strategy timeline annotations) and progress (per-session completion-time points with is_pb flags per clock). A segment section also carries `attributable_to`: the ONE entity key this definition is a subsection OF, or `null` when two or more could equally claim it — either because the definition names several parents or because another enabled definition fires identically under a different one (`tracking/segments.py::attributable_parent`). The practice log reads it to decide whether a piece may earn its parent a card the parent did not earn itself, and the projector asks the same question before letting a piece's success move the target. |
...
| `POST /api/segments` `{name, start_triggers, end_triggers, guards?, waypoints?, category?, match_mode?, parents?}` | Create a new definition; validated against the trigger vocabulary (unknown type or missing required param → 409). `waypoints` is an ordered list of middle any-of clause-sets (`[]`, the default = a plain start/end pair). `match_mode` (`"loose"` default \| `"strict"`) picks the armed-branch matcher: loose stays armed through star grabs, key grabs and level changes until the end trigger fires; strict cancels the moment anything happens that isn't the next expected step. `category` is a free-text library-grouping label (`null` = ungrouped) with no effect on matching. Triggers a full re-projection — new definitions retroactively surface every past occurrence already in the journal. |
| `PUT /api/segments/{id}` `{name?, enabled?, start_triggers?, end_triggers?, guards?, waypoints?, category?, match_mode?, parents?, clock_start?}` | Partial update (merged with the stored definition before validation). `waypoints: []`, `category: null` and `parents: []` are explicit clears, distinct from omitting the key entirely (`exclude_unset`). `parents` (plural since round 20) is the list of entities this is a SUBSECTION of — `star:<course>:<slot>` / `segment:<id>` / `area:<node>` keys, `[]` = top-level; one piece can belong to several stars. The recorder's re-record save is this endpoint with the full definition — replace in place, so routes/PBs/attempts keep their reference. Triggers re-projection. Disabled definitions stay targetable. |
| `DELETE /api/segments/{id}` | Delete a definition and cascade-delete its PBs. Triggers re-projection. 404 if not found. **Deleting a SHIPPED default is durable**: its seed key is tombstoned so the startup reconcile does not re-insert it (it stands down if a row with that key ever exists again). |
...
| `POST /api/attempts/{id}/strat` `{strat_tag}` | Reclassify one recorded attempt's strategy (`null` = no strategy); journaled, the row's own column retagged directly (no journal replay — that cost ~1 s on a live-sized journal, 2026-09-01), moves any PB the attempt saved, and broadcasts `attempts_invalidated` so every open page refetches. Older rows edit history only; reclassifying the entity's NEWEST non-cleared attempt also sets the active strategy ("that run was actually X" = "X is what I'm practicing"; `null` never propagates). `POST /api/strat` sets what to practice next directly |
| `POST /api/pb` `{attempt_id, timer_mode}` | Save a personal best from a success attempt. A save only counts while its attempt is **visible**: hide the attempt and the PB stops grading, whichever way it was hidden (`/clear`, or a validity-bounds change auto-ignoring it). **A PB is per (target, strategy)** and may only be saved under the strategy the entity is currently being practised with: 409 with `foreign_strat` when the attempt carries a different (or no) `strat_tag`, and with `no_active_strat` when the entity has no strategy selected at all. Change the attempt's strategy (`/api/attempts/{id}/strat`) or the entity's (`/api/strat`) and retry — a PB no ladder can grade is unreachable by design |
| `POST /api/pb/undo` `{attempt_id, timer_mode}` | Undo the attempt's PB save (409 unless it is the current PB **for its own strategy**, and unless that strategy is the active one) — the previous save on that strategy becomes current again. Per strategy since 2026-08-20: a later save on ANOTHER strategy no longer buries this one |
| `POST /api/import/manual` `{entity_key, strat_tag, time_cs, game_version?}` | Record a personal best you set before this tool watched you — an **imported time**. `entity_key` must name a star (`star:<course>:<star>`) or an existing local segment (`segment:<id>`); the entity's clock is selected automatically (normally IGT for stars, RTA for segments). The time is converted to frames by rounding UP to the next displayable centisecond, and lands **only if it beats your current best for that target and strategy** — so pressing this twice costs nothing. `game_version` (`"us"`/`"jp"`) is the ROM that set it and is what the rank grades against; omit it to grade on the running version. Returns `{source, found, imported, already_faster, unmappable, without_strategy, rejected, held}` (both lists are always `[]` for this door — nothing here can be dropped or held for a placement reason, unlike the sheet door below). Each landed time IS journaled — one `time_imported` event — and the projector derives a real practice-log attempt row from it, so it can be deleted or undone exactly like any played attempt (this reverses the tool's original v1 rule, which read "no attempt is created, it would break replay"). Every entity a batch lands on that has NO active strategy also gets its fastest current PB's strategy auto-selected — a normal journaled strategy pick, so it survives replay and dies with a lifetime `kind:"all"` wipe; an existing selection is never displaced. Applies to every import door |
| `POST /api/import/sheet` `{runner, refresh?}` | Land a whole runner's Ultimate Sheet column as imported times. `refresh` defaults to **true** and re-downloads the live sheet first, so what lands is their most recent entries (503 with a reason if the sheet is unreachable — never a silent empty column); `false` reads the bundled snapshot. Names come from `GET /api/library/runners`. Approach rows only. The sheet's six Bowser rows (each stage's Course = its No Reds pipe-entry movement, Battle = Bowser 1/2/3) land on THIS database's row for the same seeded movement, resolved by seed_key, on the RTA clock. Subsections, targets with no entity key, a Bowser movement this database no longer holds, and a star row the sheet times on a real-time clock are HELD rather than dropped (round 28, 2026-09-04: "maximize compatibility with the sheet") — kept beside the journal as **held times** under this source, so the column export prints them back as written, their Library rows show them, and linking a row lands its time at once. `held` NAMES every one — one row apiece, `text` = the target (with its ROM version where the sheet opens one per version), the approach or piece where it says more than the target, and the sheet's own time (`{text: "Bowser in the Sky Course — 0'48\"00", reason: "segments", row_key}`), the same `{text, reason}` shape every door's `rejected` uses (`reason` is one of `subsections`, `no_entity`, `segments`, `real_time`). They were counted by kind until round 3 (2026-08-23), which hid exactly the rows he wants to review; `rejected` is always `[]` for this door now. **A refresh that lands a newer sheet also re-derives the sheet-fitted rank standards** (round 33: every star approach with a fitted ladder, under the slot the import files it in — the star's own row is its Standard, a matched row its vetted twin's name — plus the user's adoptions; vetted ladders still win on read) and absorbs the re-grade so no celebration fires for it. **A runner's own platform legend is read back** (round 29 item 2): where rows 2 and 3 of the runner's column read "Emu" and "N64" in two different fills, every timed cell wearing one of those fills (or a near shade, within a small RGB distance) lands with that platform stamp on its attempt, so the column export can colour it back the way the runner had it; a column with no such legend — 471 of 473 on 2026-09-04 — stamps nothing, and an absent stamp reads as the emulator. Returns the manual summary plus `{rejected, held, sheet_revision}` |
| `DELETE /api/import/{source}` | Erase every imported time one source brought (`manual`, `link`, or `sheet:<runner>`). The `time_imported` journal rows are deleted and the journal replayed, so the attempt rows vanish and their PBs go with them; whatever each PB superseded becomes current again — the same restoration `/api/pb/undo` performs for one row. The held times that source kept aside go with it. Returns `{removed}`, the number of times erased |
| `GET /api/scorecard` `?scope=` | The **scorecard** for one scope — `overall` (default: the community template's full card — 15 course rows of SIX tiles each in COURSE order, closing with TWO specials rows (round 21) -- `Secret`, the seven secret-stage stars then any included castle star, and `Bowser`, each reds star followed by its fight, paired by seed key -- the set collectively holding all 120 stars including the castle's Toad and MIPS stars; every row carries `kind`: `course` | `secret` | `bowser`), `route:<id>` (the route's entities BUCKETED into the same shape — each entity joins its course's row, and Bowser-fight segments plus everything courseless join the one specials row that closes the set; course rows run in COURSE order, not the order the route touches them (round 20: "the order should be displayed in COURSE order"), while the LINES inside a card always run in template order — a course's stars by slot (the game's own star-select order), a specials star by its template position, segments after the stars — "whatever is in the scope is what we generate a scorecard for") or `course:<id>` (that course's own row; `course:0` is the five castle-secret stars); an unknown scope is a deliberate `404`, exactly like `/api/marelo`. THE CELL RULE (his canonical pairing): a course's 100-coin star is not its own cell — wherever a row holds both, the companion star's cell becomes the combined `"<companion> + 100c"` cell carrying the 100c ENTITY (usually the reds star; CCM/LLL/SSL/TTC/RR pair differently, `ranks/scorecard.py::hundred_coin_companion`), and the separate 100c cell drops; a companion in scope without its 100c keeps its own cell. The specials row leads with all the secret stars — PSS ×2, Secret Aquarium, the three Bowser REDS stars ("goals based on red coins times, not the BitS entry for now"), Wing, Vanish, Metal, WMotR. Every cell the scope's own RATING ignores is dropped here too — the card reads `service.rank_excluded()` (`ranks/scopes.py::default_excluded`: every segment outside the Bowser-fight / 100-coin-exit categories and the three Bowser course entries, plus `UNTRACKED_CASTLE_STARS` — the castle's Toad ×3 / MIPS ×2, "ignored in everyone's trackers", which return to the specials row when explicitly included), so a movement `/api/marelo` leaves out of a route's rating draws no cell, and a step that was only such movements draws no row. Including one back through `POST /api/marelo/exclude` puts its cell back, and the card's own per-row × writes that same exclusion for every cell in the row — one shared set, so the card and the rating can never list different things. Each tile is `{key, label, clock, you_cs, goal_cs, delta_cs, strat}` (the per-tile `you_rank`/`goal_rank` caps left with their toggle in round 23) — `strat` is the tile's ACTIVE strategy name or `null`, stamped through the same `ActiveStrats` resolver every other surface asks, so the card's library links can land on the approach the player practises; and `goal_source` is the INDEX into a `multi` goal's own `sources` of the pick whose time won this tile (`null` for every other goal kind, and for a tile no source covers) — recorded where the merge compares offers, so the card's coloured attribution dot and the number beside it cannot disagree. Ties keep the earlier source — each row a `sum` (`{you_cs, goal_cs, delta_cs, counted, total}`, only tiles with both sides count); `total` runs the same sum over the whole card, and the payload echoes `scope`. `you_cs` is your FASTEST current PB on that entity (igt for a star, rta for a movement) across every strategy and every region `regions` names — the same minimise-then-merge a runner goal runs on the sheet side, which is what makes a runner you imported grade himself at exactly +0.00 (rounds 33 and 34: the strategy-blind latest save let a slower strategy, and then a merged (JP)/(US) sheet row's second ROM, shadow the faster row); a PB with no ROM stamped grades on whatever is running and counts under any region choice; `goal_cs` resolves the persisted goal (below) per tile, `None` on a tile the goal can't grade (no ladder for the asked tier); a division thinner than one integer centisecond — real near the fastest tiers, where a whole tier can span 3cs — snaps to the slowest time grading AT LEAST that division rather than going empty (round 12: "We should never be missing a tier like this in our system"). Also returns `goal` (the persisted manual choice, or `{kind: "automatic", tier, division}` derived one subdivision above this scope's current MARELO rank, capped at Mario I; this read never saves a choice or changes celebration watermarks), `goal_coverage: {covered, tiles}` — tiles with a goal time over every tile on the card — `custom_goals: [name, ...]`, every SAVED custom goal's name (independent of which one, if any, is active — the picker's own "Custom" group), and — round 24 — `regions` (the ROM regions a RUNNER goal may offer a time from, a non-empty list in `us`,`jp` order) plus `detected_region` (the session's own graded version, so the control can say whether the pick agrees with the ROM). A `kind: "division"` goal grades every tile whose ladder defines that tier; a `kind: "runner"` goal grades only the tiles that runner has a sheet time for, in the regions `regions` names — with both on, the FASTER of that runner's two regional times per entity wins (`library/ratings.py::runner_times` read once per region — absent, never zero, so `goal_coverage.covered` can legitimately land short of `tiles`); a `kind: "custom"` goal grades exactly the entities its own saved `times` map names, filtered to keys the card still carries, and needs no ladder or sheet lookup at all; a `kind: "multi"` goal resolves every source through those same resolvers and keeps the FASTEST offer per entity — two levels, both his words (round 16): WITHIN an option, the fastest that option has on that star across every strategy (`runner_times` already minimises over every approach that maps to the entity), and ACROSS options, the fastest of those. Coverage is the UNION, which is how several runners cover between them the stars none of them covers alone. On an instance with a database but no live standards (`service.ranks` is `None`) the card still answers 200, with an empty goal map — a `custom` goal still resolves there, since it needs no ladder lookup at all. On a genuinely **broadcast-only** instance (no database at all — `service.db` is `None`, e.g. a second instance that lost the db lock) every route here, including this one, answers `503` rather than crashing |
| `PUT /api/scorecard/goal` `{kind: "division", tier, division} \| {kind: "runner", runner} \| {kind: "custom", name, times?} \| {kind: "multi", sources} \| null` | Set the one persisted manual scorecard goal; an empty/absent body or empty multi-source list clears it and restores automatic mode — server-side (`ui_state` KV `scorecard_goal`) so the browser and the desktop GUI agree. `kind: "division"` needs `tier` in the rank ladder and `division` in `"V"`-`"I"`, excluding the unbounded Iron V floor (422 otherwise); Iron IV–I are finite Capless goals; `kind: "runner"` needs a non-empty `runner` name, graded against the CURRENT Ultimate Sheet snapshot on the next `GET`; `kind: "multi"` takes a `sources` list of goals in the SAME shapes as above (never another `multi`), each validated by the rules its own kind is held to — so several picks can never smuggle in a tier or a saved name a single pick would refuse; `kind: "custom"` needs a non-empty `name` — with `times` present (`{entity_key: goal_cs}`) it SAVES that map under `name` in a second KV (`scorecard_custom_goals`, creating the name or overwriting it if it already exists) and activates it; with `times` omitted it SELECTS a goal already saved under that name (404 if none was). No broadcast — the goal picker is the only writer, and the card refetches like the rest of the Rank tab. Returns `{goal}`, the value just stored |
| `PUT /api/scorecard/regions` `{regions: ["us"] \| ["jp"] \| ["us","jp"]}` | Which ROM regions a **runner** goal may offer a time from — round 24, his mechanism verbatim: "your region is toggled on by default. You can also toggle the other one on. There must be at least one region enabled at all times... When combined for the scorecard, we simply take the faster time across both regions." Server-side (`ui_state` KV `scorecard_regions`) beside the goal, so the browser and the desktop GUI agree. An empty list or an unknown region is a `422`, never a silent fall back — "show me nothing" and "follow my ROM" are different intentions and only the second is reachable, by never having written this KV at all: while it is absent the card follows `detected_region` live, and a deliberate pick outlives a later detection change. Stored in `us`,`jp` order so one choice has one spelling. Division and custom goals are untouched by this — neither reads the sheet. Returns `{regions}` |
| `GET /api/scorecard/column` | The reverse of `POST /api/import/sheet` — your PBs written out as the Ultimate Sheet's own column, one line per live worksheet row, ready to paste back into the sheet. Always a live fetch (503 with a reason if the sheet is unreachable, same wording as the import door). Line 1 IS worksheet row 2, so pasting into your column's row-2 cell puts every time back on the row it came from. It prints a time exactly where `POST /api/import/sheet` would land one. **A row that names the THING — a star's own row, or any subsection — prints your best time on it however you set it, among the times no OTHER row of its block claims** (round 25: the export asked every row for the SHEET's strategy name, and times you PLAYED are filed under the names you picked, so on a real database of 268 PBs it answered 3 of 399 asks; the same run answers 44 now. Round 28 narrowed the blind ask to the leftovers: it used to answer with the star's latest save, which for an imported column is a sibling row's time printed twice). A row that names a STRATEGY still means only times set that way, and the rule naming each row's slot is `library/adoptions.py::sheet_strategy`, the import's own — a 100-coin route's sub-row is qualified by its route and a repeated name by the approach it sits under, so every worksheet row of one entity has a slot of its own. A row nothing answers prints the **held time** an import kept aside for it, if any (`POST /api/import/sheet`). Otherwise, under the same strategy name: a subsection, a castle-movement approach or a Bowser row needs that door's own facts to vouch for it (an explicit Library link, an unasked name match, or a Bowser row's seed key), and an approach on a star otherwise files under its vetted `matched_strategy`, or under the sheet's own row name where no match names one. A name this database holds no PB under is `""` — the column can miss a time, never print a wrong one. A line whose row names an explicit ROM version and whose PB was set on the other one is also `""`. Returns `{lines: [...], cells: [{text, platform, legend?}], sheet_revision, mapped, total_rows}` — the column OPENS with two **legend** cells (round 30: worksheet rows 2 and 3, `EMU` / `N64` in their platforms, marked `legend: true`, written only where those rows hold no data row so a legend can never print over a time; `mapped` does not count them), and `cells` (round 29 item 2) is `lines` with the machine beside each time: the answering PB's **platform stamp** through its attempt, resolved once here so every timed cell names `"emu"` or `"n64"` and an empty cell `null`; the clipboard's HTML half paints each timed cell by it with the stored sheet style, and an import lands a cell's platform back from the runner's own legend (see `POST /api/import/sheet`) |
| `POST /api/import/sheet/job` `{runner, refresh?}` → `{job_id}` · `GET /api/import/sheet/job/{job_id}` | The same import as `POST /api/import/sheet`, run on a background thread and REPORTED as it goes — round 29, his words: "we should have a similar progress bar, like the one we made for the copy sheet column button. I want to see my progress as it's happening, otherwise it feels laggy and unresponsive." The status is `server/jobs.py`'s shape, shared with the column export — `{state, progress, message, result}` — and `message` names the step the import is actually on (downloading the sheet, building the library, fitting the ladders, matching the runner's rows, landing the times); on `done`, `result` is byte-identical to the one-request door's body. A sheet that cannot be read ends the job in `state: "error"` with the same "could not read the sheet: …" sentence the one-request door 503s with; an unknown job id is a `404`. The one-request door is unchanged and stays the documented, testable door |
| `GET /api/scorecard/sheet_style` · `PUT /api/scorecard/sheet_style` `{emu_fill, n64_fill, font_color, font_family}` | How a pasted sheet column is COLOURED (round 29 item 2 — "people like to color the cells either an EMU color or an N64 color... The user should be able to select a color for N64 times, and select a color for EMU times... you should also be able to choose the font color"). One stored preference: the fill a cell wears when its time was set on the emulator, the fill for a console time, the text colour, and the font family, edited on the Rank tab's exports row to the right of the copy button (round 30 moved it out of Settings — "The color pickers / text pickers should live here on the scorecard itself"): two legend cells reading EMU / N64 in their fills (the preview, and exactly what the paste's first two rows are), three circle swatches, a font dropdown whose every name draws in its own face (Google Sheets' font menu, THE list — no typing, so no empty value), and Reset while anything differs. `GET` returns `{style, defaults}` — `style` is what applies now (the defaults wherever nothing is stored), `defaults` what Reset returns to. `PUT` takes three `#RRGGBB` colours (stored upper-cased) and a font name of 1–64 plain characters (it is written straight into an inline style, so CSS punctuation is refused), 422 otherwise. Defaults: `#4F7BE0` / `#E07A2F` / `#FFFFFF` / Trebuchet MS. The Copy sheet column button reads this at COPY time, so a change paints the very next paste |
| `POST /api/scorecard/column` → `{job_id}` · `GET /api/scorecard/column/{job_id}` | The same column as the GET above, run on a background thread and REPORTED as it goes — round 26, his ask: "a status line that updates at every step of the process... Right now it feels like lag, but I know that's just how long it takes to confirm things." The status is `server/jobs.py`'s job shape (itself `compare/service.py`'s, verbatim) — `{state, progress, message}` — plus `result`, which on `done` is byte-identical to the synchronous GET's body. `message` names the step the export is actually on (reading the sheet, reading its rows, building the library, matching your times) and, once done, how many rows are ready, which worksheet rows they cover and how many carry a time (the build's sentence, not the copy's: since round 29 the page holds the column and the clipboard write is the button's own gesture). Since round 33 the bytes this job downloads also REFRESH the library (kept only if newer, like `POST /api/library/refresh`) and re-derive the sheet-fitted rank standards — the export is one of the moments the app has the freshest sheet in hand. An unknown job id is a `404`. The synchronous GET is unchanged and stays the browser-reachable door |
| `GET /api/scorecard/export.csv` `?scope=` | The card itself (the same scope resolution as `GET /api/scorecard`), flattened to the template sheet's own layout — `Course,Star,Record,Goal,You,Delta`, one row per tile in card order, a `Stage RTA` row closing each course row (summing that course's own tiles; the Secret row gets no Stage RTA, it is not a stage), and one final `Upstairs RTA` row summing the whole card. `Record` is the fastest time anybody has recorded on the Ultimate Sheet for that entity across every strategy (`library/examples.py::sheet_best`, the minimum) — read from the CACHED library snapshot, never a live fetch, so this costs no network call; blank where the sheet has nothing. `Goal`/`You` are the tile's own `goal_cs`/`you_cs` from `GET /api/scorecard` (blank on a row/sum with nothing to show); `Delta` is signed (`+`/`-`). Times are the sheet's own notation (`sheet_time`, e.g. `1:21.00`); UTF-8, CRLF line endings (RFC 4180), `Content-Disposition: attachment; filename="scorecard.csv"`. The card's own Copy buttons fetch this and copy the text to the clipboard rather than navigating here — this URL exists so the file is still reachable by hand from a browser |
...
| `DELETE /api/routes/{route_id}` | Delete a route. Same durable-deletion rule as segments. |
...
|---|---|---|---|
| `GET` | `/api/ranks/standards` | `?entity=<key>&version=us\|jp` | One entity (`star:c:s` / `segment:id`): `strategies` + tiers and `estimated_strategies` (strategy to estimate provenance), RESOLVED on `version` — the game version whose standards you want to read; absent, the **grading version** (what the Game version setting resolves to, see `/api/mode`). Every version-dependent key (`strategies`, `overall`, `overall_owners`, `sole_overall_owner` — the one strategy setting every Overall cutoff, or null — `cutoff_videos`) resolves on it; `version` echoes what was resolved and `grading_version` says what grading is on, so a client can say "Viewing JP standards · you are graded on US" when they differ. `strategies_jp` is the JP-resolved `strategies` regardless of `version` (the editor draws US and JP side by side from one fetch) and `jp_strategies` lists the strategy names whose JP ladder actually differs (a JP time is annotated: the vetted seed's sparse `jp_strategies`, a sheet-fitted JP ladder, or a time typed into the editor); every other strategy is one ladder for both versions (combined unless annotated). 400 on any other `version`. Plus `overall` (the entity's OWN ladder -- `ranks/scoring.py::best_ladder`'s pointwise best across every strategy, in SECONDS like `strategies`, which is what `views.entity_rank` grades and therefore what ranking up overall costs) and `overall_owners` (`{rank: [strategy, ...]}` -- which strategies actually SET each of those cutoffs; several on a tie, and a tier can be owned by a different strategy than the one above it, which is the whole reason the pointwise best is not any single strategy's ladder). Both are derived server-side so a client never writes a second pointwise minimum. Plus `fitted_strategies` (`[name, ...]` — which keys of `strategies` are Ultimate-Sheet-derived rather than community-vetted, `ranks/standards.py::fitted_strategies`; a sibling list rather than a per-strategy bool since a "fitted" key inside one strategy's ladder would collide with its own tier names), `cutoff_videos` ({strat:{rank:url}} — fastest WATCHABLE example per tier, auto-banded from clips + library entries + user overrides; every community pool is filtered through the video-liveness verdicts (`library/videocheck.py`, shipped as `video_checks.seed.json.gz`), so a dead video falls back to the next eligible one or to no link — user overrides are never filtered), `clips` ({strat: [[time_cs, url], ...]} — the RAW liveness-filtered example pool those links resolve from: the vetted xcams clips plus every library entry with a time and a video for the row that grades this strategy, `library/examples.py`; the client bands these into per-SUBDIVISION examples itself via `librarymodel.js::bandsOf`, so the standards table's expanded rows and the Library page file a clip identically), `library_urls` (which of those URLs are LIBRARY entries — the client deep-links those times into the Library's exact entry card and leaves the rest as plain external video links), `sheet_best` (`{strat: {time_cs, runner, video}}` — the fastest time anybody has recorded on the Ultimate Sheet for each strategy, `library/examples.py::sheet_best`; the standards table draws it as one row under every rank, because the top of a ladder is not the top of the sport. Resolved through the SAME rows as `clips`, but an entry with no video still counts: the question is what the fastest recorded time IS. A strategy the sheet has nothing for is simply absent. A dead video costs the LINK (`video` comes back null) and never the row — the opposite trade from `cutoff_videos`, where the link is the payload), `user_videos` (raw overrides), `videos` (primary), `seeded` (the community-seed strat names — the custom-vs-default distinction), `xcams_url` (Daily Star page), and — for a 100-COIN star only, `[]`/`{}` for everything else — `strategy_groups` (`[{label, exit_star, strategies: [{name, leaf}]}]`, the server's own resolution of which exit-star variant each strategy belongs to), `exit_variants` (`{label: star_id}`) and `exit_star_options` (`[{star_id, name, label}]`, every star the run could end on, `label` null where the community publishes no times for that ending). A client renders `strategy_groups` and never re-derives grouping: a 100-coin strategy's stored name is variant-qualified because CCM's two variants both define "Standard". Omit `entity` for all. |
| `PUT` | `/api/ranks/standards/{entity}/{strategy}/{rank}` | `{"seconds": N}`, `?version=us\|jp` (default us) | Set one threshold. `version=jp` writes the strategy's JP time for that rank (its JP overlay — the base ladder still applies to every rank with no JP time); 400 on any other version. Broadcasts `rank_standards_changed`. |
| `DELETE` | `/api/ranks/standards/{entity}/{strategy}/jp` | — | Drop the strategy's JP overlay (the editor's "JP timed differently" toggle turned off): both versions grade on its base ladder again. A sheet-fitted JP ladder is not the user's and stays. Broadcasts `rank_standards_changed`. |
...
`rank_standards_changed`, `rank_mode_changed` and `game_version_changed` are
broadcast-only (no journal row; every open client refetches the session view
on them).

### Game version

`GET|PUT /api/mode` — the game version setting (core/modes.py, persisted as
`tracker_mode.json` beside the db): `{mode, version, effective, unsupported}`.
`version` is the choice — `"auto"` (Auto-detect), `"jp"`, `"us"`; `effective`
is what it resolves to and therefore what EVERY rank is graded on: an
explicit choice wins, Auto-detect resolves to US on the emulator path (the
only ROM it supports; a front-end that reads the version off the screen —
console-support's N64 mode — will feed the same resolver). A PUT
(`{version?: ..., mode?: ...}`, 400 on unknown) applies LIVE: the standards
store's grading version flips and `game_version_changed`
(`{setting, effective}`) tells every client to refetch — no restart. The
session view carries the same pair as `game_version`, which is what the
Library's and the standards panel's JP/US switches default to. `unsupported`
is informational: the emulator path with an explicit JP has no verified JP
...
|---|---|
| `GET /api/attempts/{id}/inputs` | The attempt's track: `runs` of `{start, length, buttons, stick_x, stick_y, yaw, speed}`, `actions` of `{start, length, action, label, group}`, and `markers` of `{frame, type, label}`. All positions use the same zero-based capture axis; missing samples, including at its boundaries, remain gaps. `frames` is the full drawn extent; `attempt_frames` is the graded attempt length (IGT when known); `lead_frames` is the axis distance before attempt frame zero, including holes. `fps`, `target`, `strategy`, `buttons` (bit/name table), and `angle_units` describe the track. `stretches` contains `[axis_start, raw_start, length]` for raw-counter conversion. Optional `from_frame` and `to_frame` widen the selected capture occurrence to include clip buffers; without them the track covers the attempt alone. This numeric range cannot identify multiple counter epochs within a clip. Unresolved repeated occurrences return 409 with a reason. The active `template` is `{id, name, origin, runs, actions}`, shifted by `lead_frames`, or `null`; an unloadable template carries `error` and empty lists. Markers use the journal's moment labels and keep their true position inside a capture hole |
| `GET /api/attempts/{id}/inputs/document` | The same track as a portable text **input document** (format v2): frame-indexed, run-length compressed, and legible enough to edit by hand. Each row is the pad and then Mario — his action as its decomp word (or a hex id), his yaw in the game's units, his speed — so a document IS the exact example, not the pad alone; a row may stop after the pad (v1 files, hand-authored rows) and Mario reads as not captured. 404 when the attempt has no captured input |
...
| Route | Purpose |
|---|---|
| `GET /api/library` | The browse index: every group (course, Castle Secret Stars, Bowser Courses, the Castle Movement sections) with its targets, each carrying counts rather than data — 252 targets summarised without shipping the 44,701 entries behind them. **One exception to "counts rather than data": `approach_names`**, every way of doing that target by name (its approaches and its subsections, 631 names / 15.9 KB across the whole snapshot). It is what the Library's search box matches on besides the target's own label — his round-12 call, "typing LBLJ finds the target that documents it" — and shipping it here rather than behind a second endpoint is what lets that search run on data the page already holds instead of a request per keystroke. An index served by an older build carries no such key, and `ui/librarysearch.js` degrades to label-only rather than throwing. **`runners` is the third thing that search matches** (2026-08-10): `{runner: [target position, ...]}`, a ROSTER rather than a name list per target — a runner appears on 124 targets on average and one on 324, so the per-target shape repeats the same 448 names until it weighs 340 KB against a 70 KB index, where deduped it is 139 KB. The positions are the `groups[].targets[].index` values already in the same response, so a client joins without a second table. Absent on an older index, same graceful degradation. |
...
| `GET /api/library/target/{index}` | One target in full: its approaches and subsections, each with every recorded entry (runner, time, video) and its effective rank ladder. Linked rows carry the local `entity_key`, canonical `strategy`, US-base `ladder` and distinct `ladder_jp` when applicable; these agree with `/api/ranks/standards` for the same entity, strategy and version. Unlinked reference rows retain the fitted ladder. `ladder_estimate` explains provisional evidence. The target's `entity_key` is also local when its approaches agree. Each row also carries `row_key` (the stable name the adopt endpoints speak), `adopted` (the entity key assigned explicitly or automatically, else null) and `held` (round 28: the held times an import kept aside for this row, `[{game_version, time_cs}]`, one per ROM the sheet timed it on; `[]` on an instance with no tracker service); the body carries `adoptable` (false on an instance that mounts no adoptions store, so a client knows not to offer the link door). The entity door below decorates identically. The body also carries `matched_segment` (round 6): for an entity-less target, the user's own segment whose NAME normalizes equal to the target's label (`{entity, name}`, else `null`) -- computed per request against the live definition list, so a segment built mid-session pairs on the next page load; an explicit adoption on a row outranks it. The body also carries `ukikipedia` (2026-08-15): the Ukikipedia RTA Guide URL for this target — its identity's page (a star's own page, a 100-coin star's `<Course> 100 Coins`, a Bowser course/fight's page), else the page its sheet label names (LBLJ, Lakitu Skip, MIPS, a Toad detour), else `null`. Resolved server-side by `links.py::ukikipedia_url` against the wiki's own page list (`ukikipedia_titles.py`, refreshed by `tools/scrape_ukikipedia.py`), so a client never composes a title and a link is always a page that exists. **404** past the end. |
| `GET /api/library/entity/{entity_key}` | Every target mapped to one trainer entity (`star:1:6`, `segment:5`) — what a link from the practice log resolves. Several is normal: each `+ 100c` row is the same 100-coin star run a different way. A LINK counts as a mapping: an entity the sheet never mapped resolves to the target(s) whose rows are adopted onto it, else to the target it name-matches, and `focus_row_key` names the adopted piece (null otherwise) so the page can open it. An entity with no mapping **and no link** is a **200 with an empty list**, because "nobody has timed this" is an answer rather than an error. |
...
| `POST /api/library/refresh` | Fetch the live spreadsheet, re-derive the library, and keep it **only if it is newer** than what we already have; `{applied: false, reason}` otherwise, which is a normal outcome rather than a failure. An applied refresh re-derives the sheet-fitted rank standards for the WHOLE library (round 33), not only the user's adoptions; the real app also runs one such refresh in the background at startup (`create_app(refresh_library_on_start=True)`, main.py only), logging one line and changing nothing when the sheet is unreachable. Runs in a worker thread — it downloads ~5.6 MB and re-fits 631 ladders, and the poller shares this process. **503** when the sheet cannot be reached, carrying the reason, because "refresh did nothing" and "refresh could not reach Google" are otherwise indistinguishable. |
...
| Route | Purpose |
|---|---|
| `GET /api/library/adoptions` | `{rows: {row key: entity key}, ladders: {entity: {strategy: ladder}}, by_entity: {entity: [{index, label}]}, matched_by_name: {entity: {index, label}}}` — what you have assigned, the ladders those assignments produce, and (round 8) the two REVERSE views the segment editor reads: which target's approaches point at which segment (`by_entity`, approach assignments only — a piece link is a partial fact), and which entity-less target auto-matches which segment by name (`matched_by_name`), so that surface never contradicts the Library page's own chip with a bare "Not linked". |
| `POST /api/library/adopt` `{row_key, entity_key}` | Assign one library row to an entity, giving it that row's community-derived ladder. The strategy is named after the approach, except where the approach is named after its target (a movement usually is) — then it lands as `Standard` rather than stuttering. **409** when the row has no rank standards or the entity names its strategies by exit-star variant; each refusal carries its reason. A community-vetted strategy of the same name is NOT refused (round 6): the assignment lands -- it carries the Library's display association -- and the vetted ladder still wins the merged read, so grading is untouched, because an assignment that silently does nothing is indistinguishable from one that worked until a rank fails to appear. **400** on a missing field. Round 28: a link gives a held time its home, so any held time on the row lands NOW through the ordinary import rule and its hold is released either way; the answer carries `held: {landed, released}`. |
| `POST /api/library/adopt_target` `{target_index, entity_key}` | Round 7 -- link a WHOLE target: every laddered approach becomes a strategy on the segment in ONE request (a half-linked target is unreachable by a failed batch); rows with no ladder are reported in `skipped` with their reason and never sink the batch. **409** with the reason when the index names no target, the entity is exit-star-qualified, or no approach carries standards at all. Lands the held times on every row it linked, as `POST /api/library/adopt` does; the answer carries `held: {landed, released}`. |
| `POST /api/library/unadopt_target` `{target_index}` | Remove every one of that target's own approach assignments -- never a subsection's, never another target's rows on the same segment. Returns `{removed}`. |
...
- Every attempt records WHICH MACHINE set its time (`Attempt.platform`, `"emu"` | `"n64"`; `null` when the closing event carried no `platform` key — added 2026-09-04). Stamped from the closing event's own payload, so it re-derives on every reproject; only the console front-end writes the key, and `core/modes.py::platform_of` is the one rule that reads `null` as `"emu"`, since every row older than the stamp was closed by Project64's memory. A personal best answers through its attempt: `pbs()`/`current_pb` rows carry `platform` from the attempt they were saved from (`null` on a row with no attempt).
