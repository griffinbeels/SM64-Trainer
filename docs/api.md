...
| `POST /api/segments/{id}/origin` `{origin: "6:3" \| null}` | Pin a segment's library category, or clear it (`origin: null`) back to the value derived from its start rules. The node must exist in `vocab().origins` (400 otherwise). Stored in the `origin_overrides` ui_state KV, not the definition row, so correcting a label never flips `seed_dirty`. 404 for an unknown segment id. Broadcast-only (`segments_changed`), never journaled. |
