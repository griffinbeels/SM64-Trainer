...
| `POST /api/import/paste` `{text, dry_run?}` | Read a block of times written in whatever form the player already has them. One time per line: the target before it, the strategy after; tabs, runs of spaces, commas and pipes all separate, so a spreadsheet paste, a hand-aligned block and a CSV read the same. Names resolve against the game's own star names, the abbreviations runners type (`BoB 1`, `WF 100c`), the Ultimate Sheet's target labels, and the player's OWN segment names (those land on the RTA clock). `dry_run` reports what would land without writing — the same planner the real call uses. Returns the manual summary plus `rejected`, one entry per unusable line with its `line`, its `text` and a `reason` (`no_time` / `no_target` / `unknown_target`) |
| `POST /api/import/sheet` `{runner, refresh?}` | Land a whole runner's Ultimate Sheet column as imported times. `refresh` defaults to **true** and re-downloads the live sheet first, so what lands is their most recent entries (503 with a reason if the sheet is unreachable — never a silent empty column); `false` reads the bundled snapshot. Names come from `GET /api/library/runners`. Approach rows only: subsections, targets with no entity key, and targets mapped to a segment are dropped and counted — `rejected` is one row per kind with something dropped (`{line: 0, text: "2 segments", reason: "segments"}`), the same `{line, text, reason}` shape every door answers with. Returns the manual summary plus `{rejected, sheet_revision}` |
| `POST /api/import/link` `{url, runner?, dry_run?}` | Import from a link to somebody's own spreadsheet. The WORKBOOK says which shape it is: a copy of the Ultimate Sheet (identified by its own main tab) is read by the real reader and the named `runner`'s column taken — with no `runner`, a `dry_run` answers `{shape: "ultimate", needs_runner: true}` with an empty summary (it is what the preview FOUND, and the UI asks for the name on the strength of it) and a real run is a 422; any other grid becomes lines and goes through the paste parser, so `star | time | strat` needs no format of its own. Every tab is read, and an unreadable row is reported as `Times!5: <the row>` — its own tab and its own row number. **Only `docs.google.com` links are fetched**: the SERVER does the fetching, so any-URL would mean any URL reachable from this machine. Anything else is a 422 naming what IS supported, refused before any request is made. An unreachable sheet is a 503 that asks whether it is shared with 'anyone with the link', which is the likeliest cause by far. Returns the manual summary plus `shape` and `rejected` |
| `POST /api/import/livesplit?dry_run=&strategy=` (raw `.lss` body) | Import the GOLDS out of a LiveSplit splits file — each split's best-ever time. Raw bytes, not multipart, so the frozen exe needs no `python-multipart`. A gold is a REAL-TIME stretch of the run, so it lands on a segment you built here (matched by name, on the RTA clock) and never on a star, whose bests are Usamune IGT; a split naming a star comes back as `not_a_segment` rather than landing. `strategy` names one for the whole file. A body that does not read as XML is a 422 that says so, because "nothing landed" and "that was a screenshot" look identical otherwise |
| `DELETE /api/import/{source}` | Erase every imported time one source brought (`manual`, `paste`, `livesplit`, `link`, or `sheet:<runner>`). The `time_imported` journal rows are deleted and the journal replayed, so the attempt rows vanish and their PBs go with them; whatever each PB superseded becomes current again — the same restoration `/api/pb/undo` performs for one row. Returns `{removed}`, the number of times erased |
