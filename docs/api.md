...
- `POST /api/attempts/{id}/replay` — cut (or reuse) the attempt's clip → `{clip_url, duration_s, truncated, fps, game_fps, source, anchor_offset_s, frame_map, frame_map_source, picture_igt, pad_stamp_agreement, video_start_s, frame_times, encode, feed_match, plugin_inexact_rows, saved_path}`. `fps` is the encoded rate and `game_fps` the 30 fps SM64 logic rate, which is the frame-step unit; `source` is `buffer` or `saved`; `saved_path` is non-null whenever a saved file exists. **`frame_map` is the clip's frame map** — entry k names the raw game frame that video frame k shows, `null` where no picture named one — and there is exactly ONE way it is built: the capture layer (`plugin/gfxwrap`) stamps every picture with the frame the game was drawing, the feed log says which stamped picture each encoded frame is, and the map is those stamps less `PLUGIN_PICTURE_LAG`. `frame_map_source` is then `plugin`. A clip whose pictures carry no stamp — recorded before the layer was installed, or with it off — gets `frame_map: null` and the timeline says "Frame-exact capture is off" rather than showing a derived answer; consumers fall back to `anchor_offset_s` arithmetic, which cannot be right on every frame because capture duplicates and skips single ones. Four DERIVED maps (frame-clock edges, the capture-tagged feed series, a pixel read, PJ64's host present counter) plus a footage aligner, a clock join and a digit refit answered here until 2026-09-05; they and their payload fields (`pad_reading`, `timer_reading`, `frame_map_mode`, `frame_map_degraded`, `frame_map_inferred`, `feed_coverage`, `frame_map_aligned`/`_offset`/`_fit`/`_windows`/`_learned`) are gone. The rest: `picture_igt` (or null) is the IGT the game held when it drew each slot's picture, which the input timeline's inspector shows as that frame's time; `pad_stamp_agreement` is the clip's own check — `{pictures, agree, rows, disagreements}`, where the plugin's copied pad is compared with the input track's on the frame the map names, `pictures` is how many could be checked (a clip reaches past the attempt, where the track has no pad), `rows` the clip's whole picture count, and each disagreement is `[slot, frame, track_pad, stamped_pad]`; `video_start_s` is the clip's own first video timestamp (an accurate cut leaves a sub-frame remainder on the first picture, and every slot the browser computes counts from it); `frame_times` is every video frame's own timestamp for a clip cut from the **picture feed** (`encode: "picture_feed"` — VFR, one frame per distinct captured picture, and `ui/frame.js::clipClock` counts slots through these rather than k / fps), `null` and `encode: "cfr"` otherwise; `feed_match` is `{frames, matched, repeats, unmatched, bias_ms, residual_ms: {median, max}}` and `plugin_inexact_rows` how many presents saw zero or two display lists — together, whether the capture was healthy. Falls back to the saved file when the buffer cannot produce the clip; clips saved before 2026-06-12 lack a metadata sidecar → `duration_s` null, `truncated` false
