"""The entity key — the one door onto "which door was that".
...
from sm64_events.core.entity import Entity, entity_at
...
def entity(home=HMC_DOOR, level=6, area=3, behaviour=DOOR_BHV) -> Entity:
    return Entity(level=level, area=area, behaviour=behaviour, home=home)
...
def test_the_same_door_keys_the_same_however_the_pool_moved_it():
    ...
    assert entity().key == entity().key


def test_two_doors_in_one_room_key_apart():
    assert entity(HMC_DOOR).key != entity(MOAT_DOOR).key
...
def test_the_area_is_part_of_the_key():
    ...
    assert entity(area=3).key != entity(area=1).key


def test_a_thing_the_game_made_mid_play_cannot_be_named():
    assert entity(home=(0, 0, 0)).placed is False
    assert entity(home=HMC_DOOR).placed is True


def test_no_engaged_object_means_no_entity():
    assert entity_at(GameSnapshot(
...
def test_the_reader_names_the_object_mario_is_engaged_with():
    ...
    assert snapshot.entity_behaviour == DOOR_BHV
    assert entity_at(snapshot).home == HMC_DOOR
