...
def test_a_first_time_on_a_strategy_lands():
    plan = decide([_candidate(886)], lambda ek, strat, mode, version=None: None)
...
def test_a_slower_time_is_kept_out_and_counted():
    plan = decide([_candidate(1200)], lambda ek, strat, mode, version=None: 266)
    assert plan.landing == []
...
def test_an_equal_time_does_not_land():
    ...
    plan = decide([_candidate(886)], lambda ek, strat, mode, version=None: 266)
    assert plan.landing == []
    assert plan.summary["already_faster"] == 1


def test_a_faster_time_lands_over_an_existing_best():
    plan = decide([_candidate(886)], lambda ek, strat, mode, version=None: 400)
    assert [frames for _, frames in plan.landing] == [266]
...
def test_the_same_strategy_is_compared_not_the_star():
    ...
    def current(entity_key, strat_tag, timer_mode, game_version=None):
...
def test_an_unreachable_centisecond_rounds_up_never_down():
    ...
    plan = decide([_candidate(1501)], lambda ek, strat, mode, version=None: None)
...
def test_a_time_with_no_strategy_LANDS_and_is_counted():
    ...
    plan = decide([_candidate(886, strat="")], lambda ek, strat, mode, version=None: None)
    assert [frames for _, frames in plan.landing] == [266]
...
def test_a_strategy_less_time_is_measured_against_the_blind_best():
    ...
    plan = decide([_candidate(886, strat="")], lambda ek, strat, mode, version=None: 200)
    assert plan.landing == []
    assert plan.summary["already_faster"] == 1


def test_a_candidate_with_no_entity_is_unmappable():
    plan = decide([_candidate(886, key="")], lambda ek, strat, mode, version=None: None)
    assert plan.landing == []
    assert plan.summary["unmappable"] == 1


def test_a_nonpositive_time_is_unmappable():
    plan = decide([_candidate(0)], lambda ek, strat, mode, version=None: None)
    assert plan.summary["unmappable"] == 1
...
def test_the_version_rides_along_with_the_candidate():
    ...
    plan = decide([jp], lambda ek, strat, mode, version=None: None)
...
def test_a_slower_duplicate_inside_one_batch_cannot_shadow_the_faster_one():
    ...
    plan = decide([_candidate(886), _candidate(1200)],
                  lambda ek, strat, mode, version=None: None)
    assert [frames for _, frames in plan.landing] == [266]
    assert plan.summary["already_faster"] == 1
