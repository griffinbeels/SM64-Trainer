...
uv run python tools/score_frame_map.py --attempt N       # score a clip's frame_map against USAMUNE'S OWN PIXELS (input display on) -- offline, no emulator; the gate for any input/footage desync report. v1's baseline on clip 741: 8/44 exact, two-sided -- what forced map v2 (capture-time tagging). Never absorb a desync into a constant without running this first
