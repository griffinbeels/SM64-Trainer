...
| One-command release | `tools/release.py X.Y.Z` — preflight (master + clean tree + `gh` auth) → tests → bump `core/version.py`+pyproject → build → SHA-256 → `gh release create` (exe + `.sha256` + auto-notes). **Build runs before any tag/push** so a broken build aborts cleanly. `--dry-run` builds + checksums only |
