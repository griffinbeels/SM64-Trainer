...
uv run python tools/score_oracle.py --attempt N [--disagreements]   # THE ORACLE, and THE gate for any claim about the frame map: score a clip's map against the frame number the game itself printed into every picture (Usamune's HUD memory display of 8032D5D4 = gGlobalTimer, top-left under the lives). Read map-free through the +1 rule; --learn <npz> pools clips into the reference alphabet. 1,716 of 1,716 exact across his three 2026-09-05 clips. It never ships to a user
...
uv run python tools/remap_clip.py --attempt N [--lag N] [--dry-run]   # RE-RUN the stamp join on a clip already cut and rewrite its sidecar, so a corrected map reaches the clip that showed the bug (a cut clip keeps its baked-in map forever). --lag applies a candidate PLUGIN_PICTURE_LAG without editing code -- the way to test a constant against a real clip, which round 33's still-open 7141 needs. Offline, and --dry-run writes nothing
uv run python tools/score_pad_read.py --attempt N [--disagreements]   # READ Usamune's input display out of a clip's pixels (replay/padread.py) and score the timeline against it: SURE / AGREE / NOWHERE / MOVED plus every contradicted slot. NO LONGER PART OF EXTRACTION (2026-09-05): it scored 82-94% on three clips the oracle certified exact, every sampled disagreement its own digit confusion, and `pad_stamp_agreement` answers the same question exactly and for free. Kept as an offline instrument, as the oracle's glyph machinery, and as the seed of reading inputs off CONSOLE footage where no RAM is readable. A display-off clip refuses
...
| Which game frame a paused video picture shows, and the pad the timeline's panel draws for it -- a debugging CHAIN (read before editing any hop; update it in the same commit a hop moves) | `inputs/sampler.py`, `inputs/track.py`, `inputs/service.py`, `replay/ledger.py`, `replay/feedmap.py`, `replay/extract.py`, `replay/padread.py`, `replay/service.py`, `ui/frame.js`, `ui/components/inputtimeline.js`, and the capture layer: `plugin/gfxwrap/` (repo root), `replay/framestream.py`, `replay/pluginsource.py`, `replay/oracleread.py` | `.claude/rules/chain-input-timeline-frame.md` |
