...
uv run python tools/probe_host_present.py            # live gate, ANSWERED 2026-08-23: PJ64's HOST-side present counter EXISTS (heap; hunt at attach by signature: 30.00/s + phase wandering in runs vs gGlobalTimer -- 149 flips/449 frames while mirrors held 0) -- the +-1 footage wobble, visible from the RAM side at last. Map v4 SHIPS on it 2026-08-24: memory/present.py hunts it in production, the poller watches it, frameclock's present series answers first; score_frame_map prints which series a clip's map came from (read-only, safe beside a live session)
