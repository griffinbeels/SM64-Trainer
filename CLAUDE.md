...
uv run python tools/probe_host_present.py            # live gate, ANSWERED 2026-08-23 then CORRECTED 2026-08-25: the counter this probe blessed (0xED1F9D4) is an IMPOSTOR -- +3 every 100.0 ms, a 10 Hz FPS accumulator whose rate/phase pass every average-based test while quantising the map (his first scored clip caught it: presses -1..-8 slots). The real signature is per-tick SHAPE: +1 per frame at ~33 ms gaps (~40 such counters live, two wandering families + one edge-locked mirror); memory/present.py's classifier hunts on that and ranks by tick-time wander vs gGlobalTimer's edges. Map v4 ships on the rebuilt hunt; score_frame_map prints which series a clip's map came from (read-only, safe beside a live session)
uv run python tools/probe_present.py                 # live gate, ANSWERED 2026-08-23: the display side IS in RAM (sNumVblanks 60 Hz, the triple-buffer swap chain) and it is RIGIDLY locked to gGlobalTimer (0 phase flips / 399 frames) -- so the footage's +-1 wobble arises PAST the emulated VI and RDRAM cannot see it; the unturned stone is PJ64's HOST-side present counter (docstring). Re-run only to re-check (read-only, safe beside a live session)
uv run python tools/score_oracle.py --attempt N [--disagreements]   # THE ORACLE (round 32 item 94): score a clip's frame map against the frame number the game itself printed into every picture (Usamune's HUD memory display of 8032D5D4 = gGlobalTimer, top-left under the lives). Read map-free through the +1 rule; --learn <npz> pools clips into the reference alphabet. This is what certifies the capture layer's stamps; it never ships to a user
uv run python tools/build_plugin.py [--test-host --out <dir>]   # build THE CAPTURE LAYER (plugin/gfxwrap -> src/sm64_events/data/plugin/sm64_trainer_gfx.dll, x86 MSVC, static CRT; the DLL is committed) and, with --test-host, the 32-bit host + fake wrapped plugin that tests/test_gfxwrap_host.py drives without Project64
uv run python tools/remap_clip.py --attempt N [--dry-run] [--disagreements]   # RE-RUN hop 5 on a clip already cut (the CLOCK join, one answer per picture, the pad display's audit) and rewrite its sidecar so the open replay shows the corrected panel -- a cut clip keeps its baked-in map forever, so this is how a reader fix reaches the clip that showed the bug (item 93). --dry-run prints the timer reading and the audit and writes nothing: the regression instrument over every cached clip
uv run python tools/probe_igt_tick.py [seconds] [wait_s]   # live gate, ANSWERED 2026-09-04 by the first three stamped clips rather than by a run (the timer never ticked while it was armed): the stamps are consistent -- lag 1 on 985 of 985 mechanical slots across two clips, 0 on the third, no wobble -- so the sampler's latest-sample pair is sound. Kept as the re-check. The question it asked (round 32 item 92): WHERE in a gGlobalTimer frame does usamune_overall tick? The timer reader's join (frame = gGlobalTimer - (igt_ram - igt_shown), 16615f33) is exact only if both counters advance at the same instant; the pad is rewritten 62% through the frame by the same game iteration and the recorder's grabs land on both sides of that mark (6510's ledger: 74% before, 26% after), so a mid-frame IGT tick makes the shipped guards REFUSE the timer path on every real clip (simulated: 26% or 74% pre-tick -> refused; 100% -> every slot mechanical and ONE FRAME LATE). Needs the Usamune timer RUNNING (waits up to wait_s for it); prints distinct-IGT-values-per-frame and the tick's phase histogram (read-only, safe beside a live session)
uv run python tools/probe_pixel_read.py --attempt N       # SUPERSEDED 2026-09-01 by score_pad_read.py (per-glyph templates read 13,524/13,525 where this read 84%/49%); kept until its deletion is his call. Was map v3's own gate (round 32 item 26): learns Usamune's display glyphs off a clip and prints the reader's training accuracy -- the WIRE-IN GATE IS 99% and it read 84%/49% (Y/X) on clip 778, which is why replay/pixelmap.py is built and tested but NOT wired into extraction; 'values the track never held' printing 0 is the proof 100% stays reachable. The reader is the whole remaining distance
uv run python tools/score_frame_map.py --attempt N       # THE gate for any input/footage desync report: score a clip's frame_map against USAMUNE'S OWN PIXELS (input display on) -- offline, no emulator. Prints ONE signed VERDICT (slots the map is out, 0 = right, negative = ahead of the footage) from a glyph-weighted fit of the STICK DIGITS; the button-icon histogram below it is per-event detail only. The icon method was the verdict until 2026-08-26 and its region mask counts moving scenery as ink -- every verdict it gave alone is suspect. Never absorb a desync into a constant without running this first
uv run python tools/score_inputs.py --attempt N       # DOES THE TIMELINE AGREE WITH THE FOOTAGE, frame by frame: Usamune's readout is a function of the pad alone, so "the digit region changed" must mean "the pad changed" -- every violation is a real disagreement between footage, map and track, and each names the local shift that would fix it. No digit recognition involved. --frames reports panel frames; the mask sees only the fire, so U/D/L/R are invisible and the test is on MAGNITUDES
uv run python tools/score_pad_read.py --attempt N [--disagreements]   # DOES THE TIMELINE SHOW WHAT THE SCREEN SHOWS -- his 100% test as a number: reads Usamune's input display out of every video frame (replay/padread.py, the same code extraction runs), pins the track to it, prints SURE / AGREE / NOWHERE / MOVED and every contradicted slot. 0 NOWHERE is the reader's own gate; a display-off clip REFUSES. Offline, read-only
uv run python tools/score_picture_offset.py --attempt N   # WHICH FRAME'S PAD DOES EACH PICTURE SHOW: the whole-clip offset sweep on two channels (stick digits, button icons) plus the reader's DP moves, scored against the sidecar's map AS HANDED -- the measurement that ended round 33's two-constant history (7015: -1 on both channels, 352 of 352 icons). A peak off 0 on TWO clips is the bar for touching PLUGIN_PICTURE_LAG or DISPLAY_LAG_FRAMES; whether the picture itself is one list old is the oracle's question, not this tool's. Offline, read-only
uv run python tools/probe_clip_seek.py --attempt N [--slots a-b]   # WHICH PICTURE DOES THE BROWSER PRESENT for the timeline's seek to slot k -- hop 6 of the input-timeline chain, ANSWERED 2026-09-01: a clip whose first video pts is not 0 (5782: 0.011 s) made every seek land one picture EARLY while the map, the reader and the track were all right; the view now counts slots from video_start_s. Headless Chromium runs the SHIPPED frame.js against the cached clip -- into each frame's own span on a picture-feed clip (item 38), the grid from the first pts on a CFR one; run it before touching the map or the reader on any "panel differs from the screen" report (offline, read-only)
uv run python tools/inspect_timeline.py --attempt N --frames 30-34   # THE first tool for any timeline-vs-video report: per PANEL frame (0-based = track axis), the track pads, the map's slots, the ledger rows and their stamps; --audit walks the whole clip (row rate, torn pairs, stamp-vs-map stretches). Built 2026-08-28 on his ask ("inspect exactly what data was held for each frame of the replay I'm looking at"); read-only
...
uv run python tools/export_overlay.py --attempt N   # the TRANSPARENT input overlay for one attempt: one .mov per LAYER (stick / buttons / facing / combined), all on one canvas, one rate and one starting frame so they stack in register in an editor -- no cropping. ProRes 4444 (--codec qtrle for lossless RLE). DNxHR SQ was the original ask and cannot carry alpha at all. Needs a running server; alignment against real footage is NOT verified by the tool (record a clip with Usamune's own display on and score against its pixels)
...
| Which game frame a paused video picture shows, and the pad the timeline's panel draws for it -- a debugging CHAIN (read before editing any hop; update it in the same commit a hop moves) | `inputs/sampler.py`, `inputs/track.py`, `inputs/service.py`, `replay/ledger.py`, `replay/extract.py`, `replay/padread.py`, `replay/service.py`, `ui/frame.js`, `ui/components/inputtimeline.js`, and the capture layer: `plugin/gfxwrap/` (repo root), `replay/framestream.py`, `replay/pluginsource.py`, `replay/oracleread.py` | `.claude/rules/chain-input-timeline-frame.md` |
| **The capture layer** -- the wrapper graphics plugin inside Project64, the frame stream, the oracle reader, the installer + consent (the setup screen) | `plugin/gfxwrap/` (repo root), `replay/framestream.py`, `replay/pluginsource.py`, `replay/oracleread.py`, `core/capturelayer.py`, `server/setup_api.py`, `ui/components/setupmodal.js`, `tools/{build_plugin,score_oracle}.py` | `.claude/rules/replay-compare.md` (the CAPTURE LAYER row) |
