...
| Rank UI (badge/banner/table/route medals) | `ui/components/ranks.js` (Medal/RankBanner + registry mirror) + `standards.js` (collapsible editable table; columns = store ∪ section strategies (custom strats get empty fillable columns; × clears data, column persists while the strat is in use); each cutoff time links to its tier's example video, strat header = Mario-row/primary, Edit-mode ▶ per-cell override, section "Examples on xcams ↗" link); wired into practice.js (section banner + attempt medals), progress.js (medal nodes), routes.js + RouteFocus (per-step medals + route avg) |
